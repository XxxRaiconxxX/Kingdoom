...
export function MarketSection() {
  const [selectedCategoryId, setSelectedCategoryId] = useState<MarketCategoryId | "all">("all");
  ...
  return (
    <section className="space-y-5">
      ...
      >
        <summary className="flex cursor-pointer list-none flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex shrink-0 items-center justify-end sm:justify-start">
            <div className={`rounded-2xl bg-stone-800 p-3 text-stone-300 transition ${isTavernOpen ? "rotate-180 text-rose-300" : ""}`}>
      ...
      {featuredItems.length > 0 ? (
        <div className="rounded-[2rem] border border-amber-500/15 bg-stone-900/75 p-6 [content-visibility:auto] [contain-intrinsic-size:800px]">
          <SectionHeader
      ...
      <div className="space-y-4 [content-visibility:auto] [contain-intrinsic-size:1400px]">
      ...
      <AnimatePresence>
        {selectedItem ? (
          <Suspense fallback={<PurchaseModalLoadingOverlay message="Preparando el pedido del mercado..." />}>
            <PurchaseModal
              item={selectedItem}
    ...
    >
      <summary className="flex cursor-pointer list-none items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={`rounded-2xl bg-stone-800 p-3 text-stone-300 transition ${isOpen ? "rotate-180 text-amber-300" : ""}`}>
            <ChevronDown className="h-5 w-5" />
          </div>
        </div>
      </summary>
...
function PurchaseModalLoadingOverlay({ message }: { message: string }) {
  return (
