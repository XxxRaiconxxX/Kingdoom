...
- Se aÃ±adiÃ³ el endpoint protegido `api/auth/staff.ts` para que administradores restablezcan contraseÃ±as y revoquen sesiones.
- Se aÃ±adiÃ³ la migraciÃ³n `player_access_codes` con hash SHA-256, caducidad, uso Ãºnico y lÃ­mite de cinco intentos.
- El frontend enlaza el perfil al usuario de Supabase mediante RPC de seguridad; el acceso antiguo queda disponible durante la transiciÃ³n.
- Validado con `npx tsc --noEmit`, `npm run build` y `git diff --check`. [Codex]

### [2026-09-02] CorrecciÃ³n y AuditorÃ­a Integral de Minijuegos de la Taberna
- **Multiplicador (TavernCrash):** Eliminada la condiciÃ³n de bloqueo del botÃ³n de inicio tras cobrar antes del colapso (`status === "cashed_out" && multiplier < crashPointRef.current`), permitiendo iniciar nuevas rondas inmediatamente.
- **Cartas del OrÃ¡culo (TavernCards):** Modificado `getRandomCard(exclude)` en `src/utils/minigamesSecure.ts` para excluir la carta actual y garantizar cartas consecutivas estrictamente diferentes en la mecÃ¡nica mayor/menor.
- **Ruleta (TavernRoulette):** Corregido el cÃ¡lculo de rotaciÃ³n acumulada `targetRotation = baseRotation + extraSpins + pocketOffset` con normalizaciÃ³n modular por mÃºltiplos de 360Â°, eliminando la deriva angular entre giros sucesivos.
- **Penales (TavernPenalty):** Reparado el flujo de cobro en `PlayerSessionContext.tsx` con fallback a `updatePlayerGold` ante errores de RLS/permisos de la RPC `increment_gold` en sesiones anÃ³nimas.
- **Rasca y Gana (TavernScratch):** Diferenciado el mensaje de bono de consolaciÃ³n frente al lÃ­mite diario y asegurada la resiliencia en la compra y resoluciÃ³n de tandas.
...
### [2026-09-01] OptimizaciÃ³n de Carga de Perfil y Supabase Timeout Fix
- Se auditÃ³ y resolviÃ³ el error de conexiÃ³n y timeout (`AbortError`) al conectar perfiles de jugadores en la web.
- Se eliminÃ³ el antipatrÃ³n de consultas en cascada y sondeo previo (`detectAuthUserIdSupport`, `detectRoleplayAccessSupport`) en `src/utils/players.ts`, sustituyÃ©ndolo por consultas directas con degradaciÃ³n limpia ante errores de esquema (`42703`/`42P01`).
- Se ampliÃ³ el timeout base de 8s a 15s (`PLAYER_QUERY_TIMEOUT_MS`) y se desacoplÃ³ el enriquecimiento de roleplay a una consulta no bloqueante para evitar que metadatos secundarios interrumpan la sesiÃ³n del jugador.
- Se creÃ³ el script de autoverificaciÃ³n `scripts/verify-player-fetch.mjs` y se configurÃ³ `.env` local para resiliencia en desarrollo. [Antigravity]

## 2026-08

### [2026-08-10] MecÃ¡nica de Stock Limitado: ValidaciÃ³n de Compra AtÃ³mica y TransiciÃ³n a Sold-Out
- Se actualizÃ³ el procedimiento almacenado `purchase_market_item_v2` en Supabase y en `supabase/supabase_market_installments.sql` para validar que `(stock_sold + p_quantity) <= stock_limit` y realizar el incremento atÃ³mico de `stock_sold` al procesar la compra, haciendo la transiciÃ³n automÃ¡tica a `sold-out` cuando se alcanza el lÃ­mite.
- Se verificaron los tres niveles de la mecÃ¡nica (frontend card/modal, rotaciÃ³n y backend RPC) con 15 pruebas automatizadas pasando con Ã©xito. [Antigravity]

### [2026-08-10] RotaciÃ³n de Mercado: InclusiÃ³n garantizada de Ã­tems destacados (Featured Showcase)
- Se corrigiÃ³ `getMarketRotationState` en `src/features/market/market.rotation.ts` para que los Ã­tems con estado destacado (`featured: true`) y stock disponible se incluyan siempre en la rotaciÃ³n activa del mercado y en la marquesina de ofertas, sin quedar bloqueados por el filtro probabilÃ­stico de rarezas cerradas (como el 10% de rareza legendaria).
- Se sincronizÃ³ en la base de datos Supabase el Ã­tem `CÃ³dice de la Pesadilla Viviente: El Ojo de AbadÃ³n` (`sword-codice-de-la-pesadilla-viviente-el-ojo-de-abadon`) con stock limitado activo y estado destacado. [Antigravity]

### [2026-08-06] ActualizaciÃ³n del Orden de Proveedores de IA
- Se actualizÃ³ el orden global de redundancia de IA a `groq -> gemini -> openrouter -> nvidia` en `.env` y `.env.example`. [Antigravity]

### [2026-08-03] RPC upgrade_player_business: Ampliaciones AtÃ³micas y AuditorÃ­a de Negocios
- Se creÃ³ el procedimiento almacenado RPC `public.upgrade_player_business(p_business_id, p_player_id, p_upgrade_type, p_new_value, p_cost_gold)` en `supabase/supabase_player_business_upgrades.sql` para procesar de forma atÃ³mica el descuento de oro y el aumento de nivel, producciÃ³n por hora o capacidad de almacenamiento de los negocios en Supabase.
...
### [2026-07-19] Portal Anime: IntegraciÃ³n de AnimeFLV (Completo) como nuevo proveedor
- Se agregÃ³ soporte para el nuevo proveedor `animeflvone` (`https://vww.animeflv.one/`) como opciÃ³n de reproducciÃ³n completa e independiente.
...
- Actualizado `src/features/animeHub/animeHub.remoteProvider.ts` para registrar "AnimeFLV (Completo)" en las opciones de reproducciÃ³n del cliente (`ANIME_PROVIDER_OPTIONS`) y habilitarlo en la bÃºsqueda automÃ¡tica distribuida (`PLAYBACK_PROVIDERS`). [Antigravity]
- **fix**: Corregido `buildPrimaryProviderUrl` en `server/anime/providerContract.ts` y modificado `api/anime/proxy.ts` para reenviar los parÃ¡metros opcionales `series` y `episode` a la API de Scraping al solicitar los enlaces de reproducciÃ³n. Esto soluciona la redirecciÃ³n directa oficial para evadir los 404 del reproductor integrado de AniChi. [Antigravity]

### [2026-07-19] Portal Anime: migraciÃ³n de AnimeFLV a su nuevo dominio
- Se migrÃ³ el scraper de AnimeFLV al nuevo dominio operativo `https://animeflv.or.at/` debido a que el dominio anterior eliminÃ³ todos los servidores de reproducciÃ³n de video.
- Se rediseÃ±Ã³ el sistema de selectores para soportar el formato WordPress/Yoast de la nueva web (bÃºsqueda con parÃ¡metro `?s=`, metadatos detallados y episodios incrustados vÃ­a JSON `.animeflv-episodes-data`).
- Se introdujo extracciÃ³n nativa tanto para los servidores de reproducciÃ³n (iframes decodificados de Base64) como para descargas directas (tabla de descargas), actualizando el backend `api/episode/[id].ts` para devolver de manera estructurada los objetos `stream` y `download`.
- ValidaciÃ³n: La API scraper local resolviÃ³ exitosamente la bÃºsqueda, los detalles y el listado de servidores/descargas de "Black Torch". Los cambios fueron desplegados a producciÃ³n de forma exitosa en el repositorio `Scraping-web-anime-api`. [Antigravity]

### [2026-07-19] El Multiplicador: soluciÃ³n de condiciÃ³n de carrera al retirar (cashout)
- Se corrigiÃ³ una condiciÃ³n de carrera donde el botÃ³n "Asegurar ahora" (retirar) no detenÃ­a la animaciÃ³n de inmediato debido a que la transacciÃ³n asÃ­ncrona de base de datos (`addPlayerGold`) demoraba en resolverse, permitiendo que la animaciÃ³n continuara y colapsara el juego en la UI a pesar de haberse iniciado el retiro.
- El estado local y el ref de estado ahora se actualizan de forma sÃ­ncrona a `cashed_out` e instancian `lastWin` inmediatamente al hacer clic.
- Se ajustÃ³ `updateMultiplier` para que, en caso de colapso de la animaciÃ³n, detenga el loop y registre el resultado en el historial de manera normal, pero no sobrescriba el estado del jugador a `crashed` si ya habÃ­a retirado previamente (`cashed_out`).
- ValidaciÃ³n: `npx tsc --noEmit` y `npm run build` pasaron limpios. Riesgo abierto: ninguno nuevo; se mantiene el riesgo heredado de `increment_gold` expuesto a clientes. [Antigravity]
...
### [2026-06-30] CreaciÃ³n de subagente KingdoomFB y materiales de marketing
- Se configurÃ³ el nuevo subagente persistente `KingdoomFB` en `.agents/agents/KingdoomFB/agent.json`, especializado en marketing, posicionamiento estratÃ©gico y copywriting para redes sociales y WhatsApp.
...
- Se redactaron copys optimizados y extendidos (limpios y muy espaciados) y prompts de imagen en formato horizontal y vertical basados en una estÃ©tica anime/fantasÃ­a premium (estilo Ufotable).
- Se redactÃ³ el borrador de la primera publicaciÃ³n oficial optimizada para el feed de Facebook.
...
### [2026-06-28] Reforma EconÃ³mica y Balanceo de Minijuegos
- **[Slots]:** RediseÃ±ada la tabla de pagos (gemas x6, coronas x15, pares x1.5) y reajustadas las probabilidades de giro para fijar un RTP global de **91.5%** con 63% de probabilidad de pÃ©rdida.
...
- **[Ruleta]:** Migrada la ruleta de 25 nÃºmeros a una **Ruleta Francesa estÃ¡ndar de 37 nÃºmeros (0 al 36)**. El pleno ahora paga un **x35** real (RTP 97.30%) y se reajustÃ³ la grilla del frontend a 6 columnas incluyendo la celda del 0 al tope.
- **[Minijuegos Seguros]:** Limitado el RTP de *RouletteSecure* al **94.0%**, reducida la racha de cartas a incremento **0.15** y habilitados colapsos instantÃ¡neos en 1.01 para *CrashSecure* (5% de chance).
...
- **[Base de Datos & Alexander]:** Ejecutada la detracciÃ³n de 900M de Alexander (balance inicial de 90M), aisladas las cuentas de administradores en `purchase_market_item` y creada la funciÃ³n `apply_wealth_tax` (tasa del 0.2% y 0.5% diario) colectando **18,311,444 de oro** en su primer ejecuciÃ³n.
- **[ValidaciÃ³n]:** `npx tsc --noEmit` y `npm run build` completados con Ã©xito. Commits y pushes realizados en ambos repositorios. [Antigravity]

### [2026-06-27] Fix: BotÃ³n de Retiro Inactivo en TavernCrash
- Se corrigiÃ³ un bug grave en `TavernCrash` donde clics en el botÃ³n de retiro fallaban silenciosamente o quedaban bloqueados por condiciones de carrera introducidas en un parche anterior.
- Se eliminaron los `useEffect` que sincronizaban `updatingRef` y `statusRef`. Dichos efectos, al ser asÃ­ncronos en React, sobreescribÃ­an las mutaciones sÃ­ncronas requeridas antes de los awaits (ej. `updatingRef.current = true`), provocando que la interfaz inhabilitara el botÃ³n temporalmente o fallara en registrar el estado `"cashed_out"` antes de un colapso.
- Se ajustÃ³ el argumento de `handleCashOut` y el bindeo en el botÃ³n (`onClick={() => handleCashOut()}`) para evitar que el evento sintÃ©tico de React (`React.MouseEvent`) interfiera con el tipado interno del componente y la lÃ³gica de caÃ­da por defecto (fallback) al multiplicador local.
- ValidaciÃ³n: Build limpia (`npx tsc --noEmit` y `npm run build` OK). No hay riesgos detectados ya que se restaura el aislamiento del flujo manual contra el bucle de animaciones. [Antigravity]
...
- Se corrigieron race conditions en `handleCashOut`, `updateMultiplier` y `handleStart` mutando `updatingRef.current` y `statusRef.current` de forma sÃƒÂ­ncrona antes de los `await`, eliminando la ventana de tiempo donde React aÃƒÂºn no habÃƒÂ­a re-renderizado y un segundo click/frame podÃƒÂ­a disparar doble cobro.
- Se aÃƒÂ±adiÃƒÂ³ guardia `!updatingRef.current` en la condiciÃƒÂ³n de auto-cashout para evitar que un frame dispare el retiro automÃƒÂ¡tico mientras una transacciÃƒÂ³n de oro estÃƒÂ¡ en vuelo.
- Se sincroniza `statusRef.current = "crashed"` inmediatamente al detectar el crash point, cerrando el exploit de lag donde un frame-drop podÃƒÂ­a permitir un cashout despuÃƒÂ©s del colapso.
- Se aÃƒÂ±adiÃƒÂ³ `redrawCanvas` a las dependencias del `useCallback` de `updateMultiplier`.
...
- Se removieron los artefactos de compilaciÃƒÂ³n de Android del tracking de git (git rm --cached) y se aÃƒÂ±adieron al .gitignore para mantener la higiene del repositorio.
- Se implementÃƒÂ³ un sistema de Escrow (tabla `bot_active_bets` y funciones RPC `place_bet` / `resolve_bet`) en Supabase para evitar pÃƒÂ©rdidas de oro en minijuegos (`!trampa`, `!dados`, `!21`) si el contenedor de Hugging Face se reinicia a mitad de la jugada.
- Se aÃƒÂ±adiÃƒÂ³ un sistema de recuperaciÃƒÂ³n en el evento `ready` del bot que busca apuestas huÃƒÂ©rfanas de mÃƒÂ¡s de 10 minutos de antigÃƒÂ¼edad y devuelve automÃƒÂ¡ticamente el oro a los jugadores. [Antigravity]

### [2026-06-21] Portal Anime Proveedor por Defecto
- Se cambiÃƒÂ³ el proveedor inicial por defecto a TioAnime y se deshabilitÃƒÂ³ temporalmente la opciÃƒÂ³n de AnimeFLV (marcada como En Mantenimiento) debido a fallos en el scraper externo. [Antigravity]

### [2026-06-21] Limite de Misiones UI
- Se agregÃƒÂ³ un lÃƒÂ­mite inicial de 3 misiones visibles en App.tsx con un botÃƒÂ³n de ocultar/mostrar para liberar espacio vertical en el tablero operativo. [Antigravity]
...
2. **Registrar despues de actuar:** **SIEMPRE** que se finalice CUALQUIER cambio (incluso mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­nimo), el asistente responsable debe aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adir una nueva entrada al `Historial de Cambios` y a la memoria MCP (`kingdoom-memory.jsonl`), y asegurarse de subir ambos a Git (`git add`, `git commit`, `git push`).
...
*   **Cambios Clave:**
    *   **Delta AtÃƒÆ’Ã‚Â³mico:** Se agregÃƒÆ’Ã‚Â³ `incrementPlayerGold(...)` sobre la RPC `increment_gold` y `PlayerSessionContext` expone ahora `addPlayerGold(delta)` para cobrar o descontar oro de forma atÃƒÆ’Ã‚Â³mica.
    ...
    *   **CorrecciÃƒÆ’Ã‚Â³n del SÃƒÆ’Ã‚Â­ntoma Reportado:** El problema mÃƒÆ’Ã‚Â¡s probable era una carrera entre `refreshPlayer()` y `setPlayerGold(nextGold)` en rondas consecutivas o pestaÃƒÆ’Ã‚Â±as activas, lo que podÃƒÆ’Ã‚Â­a pisar premios ganados con un saldo viejo.
*   **Notas/Advertencias:** `setPlayerGold(nextGold)` se mantuvo para flujos donde la fuente de verdad ya devuelve el saldo final exacto. Conviene migrar gradualmente cualquier otro flujo econÃƒÆ’Ã‚Â³mico que siga escribiendo saldos absolutos.

### [2026-06-17] - [Autor: Claude (Opus 4.8)] - Proyecto hermano: kingdoom-fichas
*   **Archivos Modificados:** Repo aparte `XxxRaiconxxX/kingdoom-fichas` (no es este repo). AquÃƒÆ’Ã‚Â­ solo se deja constancia.
*   **Resumen de Tareas:** Lanzada la **v2.0** de la app **kingdoom-fichas** (asistente de fichas, APK Android). Pasada de calidad visual premium + cierre de funciones; APK de distribuciÃƒÆ’Ã‚Â³n generado.
*   **Cambios Clave:**
    *   **[v2.0 push + tag]:** commit y tag `v2.0.0` en `github.com/XxxRaiconxxX/kingdoom-fichas` (rama main). `versionCode 2` / `versionName 2.0`.
    *   **[UI premium]:** fuentes Cinzel + Inter empaquetadas, rediseÃƒÆ’Ã‚Â±o de identidad Kingdoom, medidores de estadÃƒÆ’Ã‚Â­sticas/poderes, estados de vacÃƒÆ’Ã‚Â­o/error/listo, mobile-first con safe-areas Android, favicon herÃƒÆ’Ã‚Â¡ldico.
    *   **[Funciones]:** AnÃƒÆ’Ã‚Â¡lisis con IA usando el endpoint `analyze-ficha` de ESTE repo (Vercel) + sync del Grimorio desde la tabla `grimoire_magic_styles` de Supabase.
...
*   **Notas/Advertencias:** Build de APK por CLI con `npm run apk:debug` (requiere JDK 17/21 ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â el JBR de Android Studio sirve ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â y Android SDK). Dependencia con este repo: el endpoint IA `analyze-ficha` ya estÃƒÆ’Ã‚Â¡ desplegado y con CORS abierto para los orÃƒÆ’Ã‚Â­genes de la app.
...
*   **Resumen de Tareas:** Arranque del proyecto **kingdoom-fichas** (app/APK que ayuda a los nuevos a crear/validar su ficha antes de enviarla al grupo de WhatsApp) + nuevo endpoint de IA en este repo para el anÃƒÆ’Ã‚Â¡lisis "asistente".
*   **Cambios Clave:**
    *   **[Nuevo endpoint]:** `api/admin/analyze-ficha.ts` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â proxy Gemini (mismo patrÃƒÆ’Ã‚Â³n que `generate-magic.ts`: `_aiOrchestrator` + `setCorsHeaders`). Recibe `{ficha, avisosLocales}` y devuelve JSON `{veredicto, resumen, sugerencias[]}`. Juzga lo que las reglas locales NO pueden: coherencia edadÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Âhistoria, raza/reinoÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬Âlore, calidad de personalidad y debilidades reales. Temperatura 0.5.
    *   **[App kingdoom-fichas]:** Vite+React+TS+Capacitor (Android). Validador local de reglas duras (stats=12, niveles de poderes=5, raza/reino del catÃƒÆ’Ã‚Â¡logo, arma/habilidades sin magia, mÃƒÆ’Ã‚Â­nimos de texto escalados por edad). Generador de ficha aleatoria vÃƒÆ’Ã‚Â¡lida. Copiar/Compartir a WhatsApp con el formato exacto de la plantilla. Sync del Grimorio desde la tabla `grimoire_magic_styles` (fusionado sobre bundle de 31 magias iniciales extraÃƒÆ’Ã‚Â­das de `extracted_powers/Poderes/`).
*   **Notas/Advertencias:** Endpoint `analyze-ficha` **DESPLEGADO y verificado** (POST real ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ 200 con JSON `{veredicto, resumen, sugerencias}`; flujo end-to-end probado desde la app en `localhost:4320`, CORS OK). `MISSION_AI_ALLOWED_ORIGINS` ampliada en Vercel con `http://localhost:4320`, `https://localhost`, `capacitor://localhost` (necesario para el APK). Detalle completo del proyecto hermano en `../kingdoom-fichas/HANDOFF.md`. `npm run build` OK en ambos lados. Pendiente del lado fichas: generar el `.apk` (Android Studio).
...
*   **Cambios Clave:**
    ...
    *   **Biblioteca IA Ligera:** El manager admin de conocimiento ya no trae `content` completo para toda la lista; ahora consume resÃƒÆ’Ã‚Âºmenes livianos y solo carga el documento completo por `id` cuando se va a editar.
...
*   **Cambios Clave:**
    ...
    *   **[Feedback tactil]:** `kd-touch` agregado a botones planos que no lo tenian ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â refrescar (Cards, Scratch, Ruleta) y CTA principal (Plinko "Lanzar esferas", Carreras "Apostar/Iniciar"). Se evito `motion.button` para no chocar con framer-motion.
...
*   **Resumen de Tareas:** CorrecciÃƒÆ’Ã‚Â³n de la posiciÃƒÆ’Ã‚Â³n del modal de compra del mercado en mÃƒÆ’Ã‚Â³viles, ajustes visuales en el Frente de Temporada, y cambio en el intervalo de rotaciÃƒÆ’Ã‚Â³n del mercado a 2 horas.
*   **Cambios Clave:**
    *   **[Market - Modal de compra en mÃƒÆ’Ã‚Â³viles]:** Se centrÃƒÆ’Ã‚Â³ el modal verticalmente en el viewport mÃƒÆ’Ã‚Â³vil y se aÃƒÆ’Ã‚Â±adiÃƒÆ’Ã‚Â³ `overflow-y-auto` al contenedor exterior para permitir el scroll del formulario en pantallas pequeÃƒÆ’Ã‚Â±as.
    *   **[Market - Causa raÃƒÆ’Ã‚Â­z transform/fixed]:** Se envolviÃƒÆ’Ã‚Â³ el renderizado de `PurchaseModal` con `createPortal(..., document.body)` para evitar que ancestros con transformaciones CSS (animaciÃƒÆ’Ã‚Â³n `content-fade-in` de `kd-stage` en `index.css`) rompieran la posiciÃƒÆ’Ã‚Â³n fija del modal.
    *   **[Profile - Frente de Temporada]:** Se redujo el tamaÃƒÆ’Ã‚Â±o de la etiqueta "Avance" en `SeasonRing` de `8px` a `6.5px` y se disminuyÃƒÆ’Ã‚Â³ el espaciado de letras a `0.12em` para evitar desbordes en el cÃƒÆ’Ã‚Â­rculo de progreso. Se eliminÃƒÆ’Ã‚Â³ la etiqueta de ayuda redundante *"El resumen detallado de la temporada queda oculto..."* en estado colapsado.
    *   **[Market - RotaciÃƒÆ’Ã‚Â³n de la tienda]:** Se cambiÃƒÆ’Ã‚Â³ la frecuencia de rotaciÃƒÆ’Ã‚Â³n de artÃƒÆ’Ã‚Â­culos (`MARKET_ROTATION_WINDOW_MS`) de 5 horas a 2 horas, adaptando ademÃƒÆ’Ã‚Â¡s el intervalo en los tests de simulaciÃƒÆ’Ã‚Â³n en `market.rotation.test.ts`.
...
*   **Resumen de Tareas:** ImplementaciÃƒÆ’Ã‚Â³n de la compra de espacios adicionales para fichas de personaje con oro.
*   **Cambios Clave:**
    *   **Base de Datos:** CreaciÃƒÆ’Ã‚Â³n de la columna `max_character_sheets` en `players` y de la funciÃƒÆ’Ã‚Â³n RPC transaccional `buy_character_slot` para la deducciÃƒÆ’Ã‚Â³n atÃƒÆ’Ã‚Â³mica de oro e incremento del lÃƒÆ’Ã‚Â­mite (mÃƒÆ’Ã‚Â¡ximo 10).
    *   **CÃƒÆ’Ã‚Â¡lculo de Costo:** Se definiÃƒÆ’Ã‚Â³ un costo plano de 1,000,000 de oro para todos los espacios adicionales (slots del 3 al 10).
    *   **Frontend & Modelos:** Mapeo de la columna y exportaciÃƒÆ’Ã‚Â³n del helper de RPC en `src/utils/players.ts`. ActualizaciÃƒÆ’Ã‚Â³n de la interfaz `PlayerAccount`.
    *   **UI de Perfil:** Reemplazo de los lÃƒÆ’Ã‚Â­mites hardcodeados de fichas en `PlayerProfilePanel.tsx` por el valor dinÃƒÆ’Ã‚Â¡mico del jugador. AdiciÃƒÆ’Ã‚Â³n de un botÃƒÆ’Ã‚Â³n de compra interactivo y feedback visual premium para la compra de slots.
*   **Notas/Advertencias:** Typecheck de TypeScript y compilaciÃƒÆ’Ã‚Â³n de producciÃƒÆ’Ã‚Â³n validados con ÃƒÆ’Ã‚Â©xito sin errores.
...
*   **Resumen de Tareas:** Pulido visual del frente de clasificatoria en el perfil del jugador para volver el frontend mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¡s llamativo y legible.
*   **Cambios Clave:**
    *   **Perfil Hero:** Se reemplazÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³ el bloque simple de rango por un `SeasonRankSpotlight` con presencia visual de tarjeta hero, lectura de temporada y resumen de actividad.
    *   **Progreso Real:** `fetchPlayerMonthlyRankSnapshot` ahora expone piso del rango actual, siguiente meta, progreso porcentual dentro del escalÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³n y avance temporal de la temporada.
    *   **UI de Temporada:** Se aÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â±adiÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³ barra animada de progreso, contador de puntos faltantes al siguiente rango y mÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â©tricas compactas de misiones, eventos y premios manuales.
...
*   **Notas/Advertencias:** `npm run build` pasÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³ correctamente. `npx tsc --noEmit` sigue fallando por una dependencia faltante preexistente en `src/features/market/market.rotation.test.ts` (`vitest` no resuelto), ajena a este cambio visual.
---
...
*   **Resumen de Tareas:** ImplementaciÃƒÆ’Ã‚Â³n de la mecÃƒÆ’Ã‚Â¡nica de Mercado Personal para ÃƒÆ’Ã‚Â­tems de rol creados por usuarios con comisiones y tasas de apariciÃƒÆ’Ã‚Â³n.
*   **Cambios Clave:**
    ...
    *   **RPC de Compra:** Se modificaron las funciones RPC de compra de Supabase (`purchase_market_item` y `purchase_market_item_v2`) para realizar transferencias atÃƒÆ’Ã‚Â³micas automÃƒÆ’Ã‚Â¡ticas de las ganancias correspondientes del vendedor al saldo de oro de su cuenta.
    ...
    *   **LÃƒÆ’Ã‚Â³gica de RotaciÃƒÆ’Ã‚Â³n:** SincronizaciÃƒÆ’Ã‚Â³n determinista del catÃƒÆ’Ã‚Â¡logo filtrando ÃƒÆ’Ã‚Â­tems mediante la probabilidad configurada en `spawn_chance`.
    *   **Interfaz de AdministraciÃƒÆ’Ã‚Â³n:** Se creÃƒÆ’Ã‚Â³ la pestaÃƒÆ’Ã‚Â±a "Mercado Personal" en `AdminControlSheet.tsx` que incluye autocompletado/bÃƒÆ’Ã‚Âºsqueda de jugadores, campos interactivos (precio, split de comisiÃƒÆ’Ã‚Â³n, probabilidad de apariciÃƒÆ’Ã‚Â³n en rotaciÃƒÆ’Ã‚Â³n), previsualizador en tiempo real de oro asignado a cada parte, y listado de ÃƒÆ’Ã‚Â­tems personales existentes.
    *   **VerificaciÃƒÆ’Ã‚Â³n:** Cobertura de tests unitarios agregada en `src/features/market/market.rotation.test.ts` y ejecuciÃƒÆ’Ã‚Â³n limpia de compilaciÃƒÆ’Ã‚Â³n de producciÃƒÆ’Ã‚Â³n.
...
*   **Resumen de Tareas:** ActualizaciÃƒÆ’Ã‚Â³n de las directrices operativas del agente a la realidad actual del proyecto.
*   **Cambios Clave:**
    *   **[DocumentaciÃƒÆ’Ã‚Â³n - Agentes]:** Se reestructuraron las secciones 1 a 5 de `AGENTS.md` para incluir el nuevo Working Directory, la arquitectura del repositorio completa (los nuevos minijuegos, vistas de administraciÃƒÆ’Ã‚Â³n y modales de pago), las reglas de negocio de cuotas e intereses de financiaciÃƒÆ’Ã‚Â³n, las nuevas tablas y RPCs de Supabase y las convenciones premium de diseÃƒÆ’Ã‚Â±o de UI/UX. Las secciones 6, 7 y 8 se mantuvieron intactas.
...
*   **Resumen de Tareas:** ActualizaciÃƒÆ’Ã‚Â³n de protocolos en AGENTS.md (bootstrap, cierre y subidas).
*   **Cambios Clave:**
    *   Se integrÃƒÆ’Ã‚Â³ la subsecciÃƒÆ’Ã‚Â³n "CuÃƒÆ’Ã‚Â¡ndo ocurre el bootstrap (CRÃƒÆ’Ã¯Â¿Â½TICO)" aclarando las fases y el flujo ordenado de una sesiÃƒÆ’Ã‚Â³n.
    *   Se expandieron las reglas de subidas y cierres en la SecciÃƒÆ’Ã‚Â³n 7, regulando la detecciÃƒÆ’Ã‚Â³n de intenciÃƒÆ’Ã‚Â³n del usuario (7.2), el mapeo de repositorios y destinos de push (7.3) y la secuencia exacta de cierre de tareas (7.4).
*   **Notas/Advertencias:** Ninguno detectado.
...
*   **Resumen de Tareas:** CorrecciÃƒÆ’Ã‚Â³n de remotos de la web en AGENTS.md e ignorado de archivos temporales.
*   **Cambios Clave:**
    *   Se corrigieron los remotos de `Kingdoom-sync` en la tabla 7.3 de `AGENTS.md` para especificar que va ÃƒÆ’Ã‚Âºnicamente a GitHub (origin).
...
### [2026-06-13] - [Autor: Antigravity]
*   **Archivos Modificados:** `AGENTS.md`, `AI_CHANGELOG.md`, `ai-memory/kingdoom-memory.jsonl`
*   **Resumen de Tareas:** AdiciÃƒÆ’Ã‚Â³n de directrices para agentes asÃƒÆ’Ã‚Â­ncronos (Jules) en AGENTS.md.
*   **Cambios Clave:**
    *   Se integrÃƒÆ’Ã‚Â³ la subsecciÃƒÆ’Ã‚Â³n 7.5 en `AGENTS.md` para regular el comportamiento del agente asÃƒÆ’Ã‚Â­ncrono Jules (bootstrap automÃƒÆ’Ã‚Â¡tico, honestidad de push en su entorno y disciplina de alcance al correr sin supervisiÃƒÆ’Ã‚Â³n).
*   **Notas/Advertencias:** Ninguno detectado.
...
*   **Resumen de Tareas:** CreaciÃƒÆ’Ã‚Â³n de la interfaz web de Subastas para jugadores con soporte transaccional y sincronizaciÃƒÆ’Ã‚Â³n en tiempo real.
*   **Cambios Clave:**
    *   **[Web - Player UI]:** Se diseÃƒÆ’Ã‚Â±ÃƒÆ’Ã‚Â³ y desarrollÃƒÆ’Ã‚Â³ el componente `PlayerAuctionPanel` con un panel de subastas activas, countdowns individuales en vivo, soporte para el envÃƒÆ’Ã‚Â­o de pujas y botÃƒÆ’Ã‚Â³n de retiro con confirmaciones.
    *   **[Web - IntegraciÃƒÆ’Ã‚Â³n]:** Se incrustÃƒÆ’Ã‚Â³ el panel en `MarketSection.tsx` bajo un nuevo bloque `<details>` premium de color ÃƒÆ’Ã‚Â¡mbar con el icono `Gavel`.
    *   **[Web - Realtime]:** Se enlazÃƒÆ’Ã‚Â³ el componente a Supabase Realtime para recibir actualizaciones automÃƒÆ’Ã‚Â¡ticas de ofertas y ganadores al instante sin recarga de pÃƒÆ’Ã‚Â¡gina.
*   **Notas/Advertencias:** ValidaciÃƒÆ’Ã‚Â³n de build exitosa (`npm run build` completado en 2m 56s).
...
*   **Resumen de Tareas:** IntegraciÃƒÆ’Ã‚Â³n completa de la mecÃƒÆ’Ã‚Â¡nica de Subastas en el Bot de WhatsApp: comandos de jugador, anuncios en tiempo real y resoluciÃƒÆ’Ã‚Â³n automÃƒÆ’Ã‚Â¡tica de expiraciones.
*   **Cambios Clave:**
    *   **[Bot - Comandos]:** Se implementaron los comandos pÃƒÆ’Ã‚Âºblicos `!subastas` (listar subastas activas), `!pujar <item / #lista> <monto>` (realizar pujas atÃƒÆ’Ã‚Â³micas mediante RPC) y `!retirarse <item / #lista>` (bloquear pujas futuras en una subasta).
    *   **[Bot - Realtime]:** Se aÃƒÆ’Ã‚Â±adiÃƒÆ’Ã‚Â³ `startAuctionsRealtime` para que el bot escuche cambios en Supabase Realtime y publique automÃƒÆ’Ã‚Â¡ticamente en WhatsApp cuando una subasta se crea, se puja, o se resuelve.
    *   **[Bot - Scheduler]:** Se configurÃƒÆ’Ã‚Â³ una tarea recurrente en el cron del scheduler que comprueba cada minuto si hay subastas expiradas para resolverlas automÃƒÆ’Ã‚Â¡ticamente llamando a la RPC `resolve_market_auction`.
...
*   **Resumen de Tareas:** Handoff de la sesiÃƒÆ’Ã‚Â³n para Antigravity 2. Todo ha sido validado, el bot fue limpiado y optimizado, y la carga inicial de la web fue mejorada.
*   **Cambios Clave:**
    *   **[Handoff / Next Steps]:** 
        1. **AtÃƒÆ’Ã‚Â³mica de apuestas:** Priorizar la refactorizaciÃƒÆ’Ã‚Â³n de las apuestas del bot (`!dados`, `!trampa`, `!21`) mediante funciones RPC atÃƒÆ’Ã‚Â³micas en Supabase (`place_bet`) para evitar condiciones de carrera y pÃƒÆ’Ã‚Â©rdidas de oro inconsistentes.
        2. **Subastas:** Validar y completar la interfaz de subastas (actualmente estÃƒÆ’Ã‚Â¡n hechas las utilidades y SQL).
        3. **Monitoreo Bot:** Observar la estabilidad del bot en Hugging Face (sin el flag `--single-process`) para comprobar la mitigaciÃƒÆ’Ã‚Â³n del reinicio.
*   **Notas/Advertencias:** El repositorio web (`Kingdoom-sync`) compila sin errores (`tsc --noEmit` y `build` exitosos). El bot (`kingdoom-bot`) tiene los cambios de optimizaciÃƒÆ’Ã‚Â³n empujados.
...
*   **Cambios Clave:**
    ...
    *   **[Bot - Estabilidad]:** se quito `--single-process` de los args de Puppeteer. Ese flag es causa conocida de `Protocol error / Target closed / Session closed` con whatsapp-web.js ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â exactamente los errores que el propio `index.js` detecta para reiniciar el contenedor. Observar si baja la frecuencia de reinicios en HF Spaces.
...
*   **Notas/Advertencias:** `node --check` OK en los 3 JS editados; sin referencias rotas (grep). Pusheado a GitHub y a Hugging Face (redeploy del Space ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â el bot se reinicio con la imagen nueva). Pendientes de la auditoria, NO implementados aun: (1) race condition en apuestas `!dados`/`!trampa`/`!21` ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â la validacion de saldo y el debito no son atomicos, requiere RPC `place_bet` en Supabase; (2) sesiones de blackjack en memoria se pierden ante reinicios con apuesta ya debitada.
...
*   **Cambios Clave:**
    ...
    *   **[Navegacion]:** micro-interaccion en la barra inferior ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â el icono de la pestaÃƒÆ’Ã‚Â±a activa se eleva 1px con scale 1.06 y transicion suave; deshabilitada bajo `prefers-reduced-motion`.
...
*   **Resumen:** RediseÃƒÆ’Ã‚Â±o completo y premium del menÃƒÆ’Ã‚Âº de navegaciÃƒÆ’Ã‚Â³n de pestaÃƒÆ’Ã‚Â±as del panel de administraciÃƒÆ’Ã‚Â³n y eliminaciÃƒÆ’Ã‚Â³n total del "Generador de Items IA / Pinterest" de la secciÃƒÆ’Ã‚Â³n Mercado del admin.
*   **Cambios Clave:**
    *   **[Admin - NavegaciÃƒÆ’Ã‚Â³n/Tabs]:** RediseÃƒÆ’Ã‚Â±o estÃƒÆ’Ã‚Â©tico y responsivo de las 10 pestaÃƒÆ’Ã‚Â±as del menÃƒÆ’Ã‚Âº de administraciÃƒÆ’Ã‚Â³n.
        - Se agregaron iconos de Lucide-React semÃƒÆ’Ã‚Â¡nticos para cada botÃƒÆ’Ã‚Â³n de pestaÃƒÆ’Ã‚Â±a.
        - Se estructuraron las pestaÃƒÆ’Ã‚Â±as en 3 grupos lÃƒÆ’Ã‚Â³gicos diferenciados con separadores visuales (`.kd-admin-tab-divider`): **GestiÃƒÆ’Ã‚Â³n** (Jugadores, Misiones, Eventos), **EconomÃƒÆ’Ã‚Â­a** (Mercado, Negocios) e **IA & Lore** (Staff IA, Magias, Bestiario, Flora, Archivo IA).
        - Se optimizÃƒÆ’Ã‚Â³ el estilo activo/inactivo con gradientes premium, sombras internas y hover interactivo suave.
        - Se aÃƒÆ’Ã‚Â±adiÃƒÆ’Ã‚Â³ responsividad: en dispositivos mÃƒÆ’Ã‚Â³viles (`< 640px`) se despliega en un grid compacto de 2 columnas para una cÃƒÆ’Ã‚Â³moda navegaciÃƒÆ’Ã‚Â³n tÃƒÆ’Ã‚Â¡ctil; en pantallas de escritorio se mantiene la disposiciÃƒÆ’Ã‚Â³n en lÃƒÆ’Ã‚Â­nea optimizando el espacio horizontal.
    *   **[Admin - Limpieza de Generador IA]:** RemociÃƒÆ’Ã‚Â³n completa del mÃƒÆ’Ã‚Â³dulo experimental "Generador de Items IA" basado en Pinterest en la pestaÃƒÆ’Ã‚Â±a de Mercado.
        - Se eliminÃƒÆ’Ã‚Â³ todo el cÃƒÆ’Ã‚Â³digo JSX que contenÃƒÆ’Ã‚Â­a el visualizador de Pinterest y el disparador de IA (inputs de URL de Pinterest, previsualizadores, feedbacks, tema de IA y botones de acciÃƒÆ’Ã‚Â³n).
    ...
    *   **[Admin - Estilos del Sistema]:** AdiciÃƒÆ’Ã‚Â³n en `index.css` de clases CSS `.kd-admin-tabs`, `.kd-admin-tab-group` y `.kd-admin-tab-divider` con transiciones fluidas y gradientes HSL.
*   **Notas/Advertencias:** Los archivos utilitarios subyacentes (`utils/marketAi.ts` y `utils/pinterestPicker.ts`) se preservaron en el repositorio para no romper posibles dependencias en API routes, pero ya no tienen acoplamiento con la interfaz de usuario. Verificado con `npx tsc --noEmit` y `npm run build` con ÃƒÆ’Ã‚Â©xito.

### [2026-06-10] - [Autor: Antigravity]
*   **Archivos Modificados:** `api/admin/generate-market-item.ts`
*   **Resumen de Tareas:** Mejora del prompt de generaciÃƒÆ’Ã‚Â³n de ÃƒÆ’Ã‚Â­tems con IA (Market Forge) para alinear mecÃƒÆ’Ã‚Â¡nicas y hacer los efectos mÃƒÆ’Ã‚Â¡s descriptivos.
*   **Cambios Clave:**
    *   **Contexto del Sistema:** Se actualizÃƒÆ’Ã‚Â³ el prompt para incluir las reglas exactas del sistema de Kingdoom: dados (d20 + stat, daÃƒÆ’Ã‚Â±o en d6), mecÃƒÆ’Ã‚Â¡nicas de mano blanca (fÃƒÆ’Ã‚Â­sica) y mano negra (mÃƒÆ’Ã‚Â¡gica/veneno), y las defensas activas (STR = Bloquear, INT = Defender, AGI = Esquivar).
    *   **GeneraciÃƒÆ’Ã‚Â³n de Habilidad (`ability`):** Se ajustaron las reglas JSON del prompt. Ahora la IA debe obligatoriamente describir el efecto mecÃƒÆ’Ã‚Â¡nico con porcentajes exactos ligados a stats (ej. 30% del STR) y definir una frecuencia clara de uso (cooldown en turnos o porcentaje de probabilidad de activaciÃƒÆ’Ã‚Â³n).
*   **Notas/Advertencias:** ValidaciÃƒÆ’Ã‚Â³n de build sin errores (`npm run build` exitoso). Las mecÃƒÆ’Ã‚Â¡nicas deberÃƒÆ’Ã‚Â­an estar mucho mejor representadas en el texto autogenerado.

### [2026-06-10] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/types.ts`, `src/utils/inventory.ts`, `src/components/PlayerInventorySheet.tsx`
*   **Resumen de Tareas:** VisualizaciÃƒÆ’Ã‚Â³n del estado de cuotas e ÃƒÆ’Ã‚Â­tems bloqueados en el Inventario del jugador.
*   **Cambios Clave:**
    *   **`src/types.ts`**: Se aÃƒÆ’Ã‚Â±adiÃƒÆ’Ã‚Â³ `isLocked?: boolean` a `InventoryEntry` y se creÃƒÆ’Ã‚Â³ el nuevo tipo `PaymentPlan` que mapea la tabla `payment_plans` de Supabase.
    *   **`src/utils/inventory.ts`**: Se aÃƒÆ’Ã‚Â±adiÃƒÆ’Ã‚Â³ `is_locked` al select y al mapeo de `fetchPlayerInventory`. Se creÃƒÆ’Ã‚Â³ `fetchPlayerPaymentPlans` que obtiene todos los planes activos/en-mora del jugador.
    *   **`src/components/PlayerInventorySheet.tsx`**: Reescrito con sistema de pestaÃƒÆ’Ã‚Â±as: pestaÃƒÆ’Ã‚Â±a "Inventario" (con badge ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬â„¢ "En cuotas" en cada ÃƒÆ’Ã‚Â­tem bloqueado) y pestaÃƒÆ’Ã‚Â±a "CrÃƒÆ’Ã‚Â©ditos" que muestra resumen (planes activos, en mora, deuda total) y tarjetas detalladas por plan (cuotas pagadas/total, barra de progreso, saldo restante, prÃƒÆ’Ã‚Â³ximo pago, dÃƒÆ’Ã‚Â­as de mora).
*   **Notas/Advertencias:** `npx tsc --noEmit` ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ exit 0. Sin errores.

### [2026-06-10] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/utils/purchases.ts`, `supabase_market_installments.sql`, `ai-memory/kingdoom-memory.jsonl`
*   **Resumen de Tareas:** CorrecciÃƒÆ’Ã‚Â³n del error "Tu cuenta segura aun no esta vinculada a un jugador del reino." al intentar comprar artÃƒÆ’Ã‚Â­culos en el mercado.
*   **Cambios Clave:**
    *   **TypeScript (`src/utils/purchases.ts`)**: Se agregÃƒÆ’Ã‚Â³ el parÃƒÆ’Ã‚Â¡metro `p_player_id: input.playerId` que faltaba en la llamada RPC a `purchase_market_item_v2`. Al omitir este parÃƒÆ’Ã‚Â¡metro en el frontend, la base de datos recibÃƒÆ’Ã‚Â­a un valor NULL y por lo tanto fallaba al resolver la relaciÃƒÆ’Ã‚Â³n de vinculaciÃƒÆ’Ã‚Â³n.
    *   **SQL (`supabase_market_installments.sql`)**: Se ajustÃƒÆ’Ã‚Â³ la firma de la funciÃƒÆ’Ã‚Â³n `purchase_market_item_v2` para aceptar `p_player_id uuid` e implementÃƒÆ’Ã‚Â³ lÃƒÆ’Ã‚Â³gica de auto-vinculaciÃƒÆ’Ã‚Â³n. Si el jugador no estÃƒÆ’Ã‚Â¡ vinculado pero el usuario estÃƒÆ’Ã‚Â¡ autenticado, la base de datos inserta automÃƒÆ’Ã‚Â¡ticamente una fila en `player_auth_links` para vincularlos en el primer intento de compra segura.
*   **Notas/Advertencias:** Se corriÃƒÆ’Ã‚Â³ `npx tsc --noEmit` y `npm run build` con ÃƒÆ’Ã‚Â©xito. La base de datos ya cuenta con la funciÃƒÆ’Ã‚Â³n actualizada.
...
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¿Ã‚Â½ de la funciÃƒÂ¯Ã‚Â¿Ã‚Â½ RPC process_market_installments para el cobro automÃƒÂ§Ã¢â‚¬â€œÃ‚Â¸ico de cuotas con reglas estrictas (1 dÃƒÂ¥Ã†â€™Ã‹Å“ de gracia, 5% mora acumulativa diaria, embargo a los 5 dÃƒÂ¥Ã†â€™Ã‹Å“s). Bloqueo de compras a crÃƒÂ©Ã‚Â¦Ã‹Å“ito limitado a 14 dÃƒÂ¥Ã†â€™Ã‹Å“s post-embargo.
...
*   **Resumen:** OptimizaciÃƒÆ’Ã‚Â³n drÃƒÆ’Ã‚Â¡stica de latencia en la lectura de mensajes del bot y adecuaciÃƒÆ’Ã‚Â³n para despliegue en Hugging Face Spaces.
*   **Cambios Clave:**
    *   **[Bot - OptimizaciÃƒÆ’Ã‚Â³n de Latencia]:** Se refactorizÃƒÆ’Ã‚Â³ el manejador de mensajes en `index.js`. La funciÃƒÆ’Ã‚Â³n `checkIsAdmin`, que ejecutaba una consulta a Supabase por cada mensaje recibido, ahora es *perezosa (lazy)*. Solo consulta la BD si el mensaje contiene un comando de la lista blanca administrativa o si el usuario estÃƒÆ’Ã‚Â¡ interactuando en el `Market Forge`. Esto reduce a cero la latencia de base de datos para trÃƒÆ’Ã‚Â¡fico estÃƒÆ’Ã‚Â¡ndar de rol.
    *   **[Bot - Despliegue en Hugging Face]:** Se confirmÃƒÆ’Ã‚Â³ el correcto funcionamiento del servidor HTTP existente en `index.js`, el cual expone el puerto definido por el entorno (`PORT` 7860), asegurando que el *healthcheck* de Hugging Face Spaces apruebe el arranque y mantenga el contenedor vivo (estado *Running*).

### [2026-06-08] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/gmTracker.js`, `kingdoom-bot/src/index.js`, `Kingdoom-sync/AI_CHANGELOG.md`
*   **Resumen:** RefactorizaciÃƒÆ’Ã‚Â³n y simplificaciÃƒÆ’Ã‚Â³n del tracker del Game Master (GM): eliminaciÃƒÆ’Ã‚Â³n de la integraciÃƒÆ’Ã‚Â³n con Google NotebookLM.
*   **Cambios Clave:**
    *   **[Bot - Limpieza de NotebookLM]:** Se eliminaron los subprocesos de Python y las funciones de aprovisionamiento de libretas en caliente. La integraciÃƒÆ’Ã‚Â³n previa resultaba inestable al depender fuertemente de cookies mediante Playwright.
    *   **[Bot - Motor Gemini Puro]:** La narrativa del GM ahora vuelve a procesarse exclusivamente con el motor base de Gemini (`askKingdoomAI`), asegurando respuestas mÃƒÆ’Ã‚Â¡s estables y sin retrasos de aprovisionamiento.
    *   **[Bot - OptimizaciÃƒÆ’Ã‚Â³n de Arranque]:** Se eliminÃƒÆ’Ã‚Â³ el loop de `autoProvisionMissions()` en el evento `ready` de WhatsApp (`index.js`), acelerando el encendido del bot y limpiando logs innecesarios.
...
*   **Resumen:** IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n completa y automatizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n del Game Master con Google NotebookLM mediante sincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡mica de grimorio y enciclopedia y aprovisionamiento bajo demanda.
*   **Cambios Clave:**
    *   **[Bot - Supabase IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n]:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de dos funciones helper robustas en `supabase.js`: `getFormattedGrimoire()` y `getFormattedEncyclopedia()`.
        - `getFormattedGrimoire()`: Consulta la tabla `grimoire_magic_styles` de Supabase, extrayendo la informaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n estructurada de hechizos, sus niveles, cooldowns, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­mites de uso, efectos y contramedidas de seguridad ("anti-mano negra"). Genera un documento en formato Markdown riguroso y jerÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡rquico.
        - `getFormattedEncyclopedia()`: Consulta la tabla `knowledge_documents` de Supabase para compilar las entradas histÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ricas, facciones, reglamentos del sistema de combate, geopolÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­tica y lore general del Reino, formateando todo en un Markdown legible.
    *   **[Bot - Provisionador Python]:** ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de `notebooklm_provisioner.py` para aceptar el payload ampliado con `grimorio_content` y `enciclopedia_content`. Este script normaliza la cookie `NOTEBOOKLM_COOKIES` en formato Playwright, crea el Notebook con el tÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­tulo `[GM] <Nombre de MisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n>` y aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â±ade secuencialmente cuatro fuentes de texto independientes usando el cliente automatizado de NotebookLM:
        1. "Reglas Generales del Game Master (GM)" (System Prompt base).
        2. "Lore e Indicaciones de la MisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n - <Nombre>" (Instrucciones especÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­ficas).
        3. "Grimorio Oficial de Magias y Hechizos" (Markdown dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡mico desde Supabase).
        4. "Enciclopedia y Codex del Reino" (Markdown dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡mico de lore desde Supabase).
    *   **[Bot - Aprovisionamiento Justo a Tiempo (On-Demand)]:** ModificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n en `gmTracker.js` dentro de `startMissionTracker()`. Al iniciar el rastreo de una misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n con el comando `!misionstart`, si la misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n no posee un `notebook_id` configurado y existen las cookies de autenticaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n, el bot genera el NotebookLM en caliente y actualiza el campo `notebook_id` en `realm_missions` mediante Supabase. Esto permite crear misiones nuevas en la interfaz administrativa web de la aplicaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n y disponer de sus libretas al instante sin reiniciar el servicio.
    *   **[Bot - SincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n al Inicio]:** ModificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n en `index.js` para ejecutar `autoProvisionMissions()` durante el evento `ready`. Busca todas las misiones en base de datos que carezcan de un `notebook_id` asociado y las aprovisiona en lotes de manera asÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­ncrona, optimizando la consulta a base de datos al recuperar el grimorio y la enciclopedia una sola vez al inicio del bucle.
*   **Notas/Advertencias:** El flujo depende de que la variable de entorno `NOTEBOOKLM_COOKIES` estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â© configurada correctamente. La generaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n en caliente requiere un tiempo extra de aprovisionamiento (~2-5s) durante la primera ejecuciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de `!misionstart`, tiempo durante el cual el bot procesa el flujo en segundo plano y asocia el ID de forma transparente para el usuario final.
...
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n del flujo de "aceptar" y "negar" para el modo multijugador PvP del Blackjack (!21) en WhatsApp.
*   **Cambios Clave:**
    *   **[Bot - Blackjack PvP Accept/Deny]:** Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ el estado "pending" a las sesiones de multijugador para esperar la respuesta de los invitados ("aceptar" o "negar").
    *   **[Bot - Timeout Pendiente]:** Si expira el tiempo mientras la sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡ pendiente, automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente se declina por los inactivos y comienza la partida con los que sÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­ aceptaron.

### [2026-06-03] - [Autor: Antigravity]
*   **Archivos Modificados:** `docs/blackjack-simulation.md` (en Kingdoom-sync), `AI_CHANGELOG.md`
*   **Resumen:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n del documento de simulaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n detallado para el minijuego de Blackjack (!21) en WhatsApp, cubriendo los flujos Solo y PvP.
*   **Cambios Clave:**
    *   **[Docs - Blackjack Simulation]:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de `blackjack-simulation.md` que detalla el paso a paso, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­mites diarios de uso, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­mites de apuesta segÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Âºn fin de semana, mecÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡nica de juego y el cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo exacto del pozo y las garantÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­as de pago del modo multijugador PvP en WhatsApp.
...
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de la modalidad multijugador PvP para el Blackjack (`!21`) por WhatsApp con control de turnos, timeout de 5 minutos y divisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n proporcional del pozo de apuestas.
*   **Cambios Clave:**
    *   **[Bot - Blackjack PvP]:** Se expandiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ `blackjack.js` para dar soporte a partidas multijugador PvP (2+ jugadores) cuando se etiqueta a otros usuarios.
    *   **[Bot - Primera Ronda con 1 Carta]:** Se modificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ la distribuciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de cartas iniciales para entregar exactamente 1 carta por jugador en la primera ronda del modo multijugador PvP.
    *   **[Bot - Interceptor Multijugador]:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ `index.js` para autorizar a cualquiera de los participantes del grupo a interactuar con el tablero enviando sus comandos de juego (`pedir` o `plantarse`).
    *   **[Bot - Autoplantado por Timeout]:** Se programÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³ un temporizador de 5 minutos que fuerza la acciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n de "plantarse" para los participantes inactivos de la ronda.
    *   **[Bot - GarantÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­a de Ganancias y Empates]:** En caso de empate, el pozo se distribuye equitativamente. Se implementaron multiplicadores garantizados mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â­nimos del sistema (`2.5x` para 21 natural, `2x` para victoria regular) por encima de la porciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n correspondiente del pozo si esta es menor.
*   **Notas/Advertencias:** Validado localmente con un script de prueba de cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo de puntuaciones y verificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â³n sintÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â¡ctica de Node.js.
...
*   **Cambios Clave:**
    *   **[Bot - Blackjack] Azar auditado:** Se reviso lackjack.js y no hay evidencia de cartas amaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â½Ã‚Â±adas. El juego crea un mazo completo de 52 cartas, aplica Fisher-Yates con Math.random() y reparte desde ese mazo barajado, por lo que una racha de 3 derrotas seguidas entra dentro de lo esperable para Blackjack.
...
*   **Resumen:** IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del minijuego !21 (Blackjack) por WhatsApp y protecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n contra interferencias.
*   **Cambios Clave:**
    *   **[Bot - Blackjack]:** Se registrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!21` en `index.js`, redirigiendo al handler de Blackjack para iniciar partidas.
    *   **[Bot - IntercepciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Respuestas]:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ un interceptor estricto al inicio de la recepciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de mensajes. Si un mensaje cita a uno de los mensajes de partidas de Blackjack activas, solo se procesa el comando (`pedir` o `plantarse`) si proviene exactamente del jugador que iniciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la partida (`sender === session.playerPhone`). Cualquier otro mensaje es ignorado completamente para evitar interferencias en grupos.
    *   **[Bot - MenÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âº de Ayuda]:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la descripciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del comando `!21 <monto>` al compendio de comandos del aventurero (`!ayuda`).
*   **Notas/Advertencias:** La validaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de sintaxis de los archivos modificados ha sido completada con ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©xito.
...
*   **Cambios Clave:**
    ...
    *   **[Diagnostico del caso]:** El flujo `draft` y `revise` funcionaba, pero `confirm` devolvia `500 FUNCTION_INVOCATION_FAILED`, seÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±al de crash al cargar ese submodulo en Vercel. El import cruzado desde `src/features/market/*` era el punto mas fragil y quedo eliminado.
...
*   **Resumen de Tareas:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del minijuego de Blackjack (`!21`) para el bot de WhatsApp con control de sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n estricto mediante respuestas.
*   **Cambios Clave:**
    *   CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de `src/handlers/blackjack.js` con la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica de Blackjack (apuestas, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mites diarios de 3 usos entre semana y 5 los fines de semana, crupier que planta en 17).
    *   La sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del juego estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ anclada a la respuesta directa al mensaje del bot para evitar interferencias en grupos.
    *   IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n con Supabase para descontar la apuesta antes de jugar y registrar/verificar el uso diario.
    *   ModificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de `src/index.js` para interceptar respuestas a mensajes activos y ejecutar el comando `!21`.
...
*   **Resumen:** MVP de forja automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡tica de ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­tems de mercado por WhatsApp con borrador IA, ajustes conversacionales, confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­cita y auditorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a en Supabase.
*   **Cambios Clave:**
    *   **[Backend - Assistant Market]:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adieron los endpoints protegidos `POST /api/admin/assistant/market/draft`, `revise` y `confirm`, todos autenticados por `WHATSAPP_ASSISTANT_SECRET` y pensados para uso exclusivo del `kingdoom-bot`.
    *   **[Backend - AuditorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a/Draft State]:** Se versionÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `supabase_assistant_admin_actions.sql` como tabla fuente de verdad para borradores administrativos. Guarda actor, rol (`admin|staff`), payload propuesto, referencia visual, confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n/cancelaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n, modelo IA y resultado final.
    *   **[Backend - IA de Mercado]:** Se creÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ un motor server-side compartido para generar y revisar drafts de ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­tems usando referencia visual + prompt del staff + contexto resumido del mercado actual. El precio puede ajustarse por conversaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n antes de confirmar.
    *   **[Bot - Flujo Conversacional]:** Se integrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `!forjaritem <idea> [url]` y `!mercado crear ...` en WhatsApp. El bot detecta una sola sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n activa por staff/admin por chat, acepta ajustes conversacionales, soporta `confirmar` / `cancelar` y publica en `market_items` solo tras confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­cita.
    *   **[Bot - Permisos]:** AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡s de admins, ahora existe `isStaffUser()` con whitelist por `STAFF_NUMBERS` para habilitar el flujo de forja a staff sin abrir el resto de comandos administrativos sensibles.
*   **Notas/Advertencias:** `node --check` pasÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ en los archivos nuevos/modificados del bot. Los endpoints nuevos de `api/` compilaron con `npx tsc --noEmit --skipLibCheck ...`. El `npx tsc --noEmit` global y `npm run build` de `Kingdoom-sync` siguen fallando por un problema preexistente de resoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de `swr` en `src/components/GrimoireSection.tsx` y `src/sections/MarketSection.tsx`, ajeno a esta implementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n. Para que el flujo funcione en producciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n deben configurarse `WHATSAPP_ASSISTANT_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` y, si se usarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡n staff no-admin, `WHATSAPP_ASSISTANT_STAFF_NUMBERS` en backend y `STAFF_NUMBERS` en el bot.
...
*   **Resumen:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de directrices exhaustivas de agentes (Personas) para el Reino.
*   **Cambios Clave:**
    *   **[Docs - Agentes]:** Se crearon 7 nuevos perfiles de contexto en `docs/agents/` cubriendo todas las ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡reas posibles del proyecto (`Kingdoom-sync` y `Kingdoom-bot`): Architect, Frontend, Backend, Minigames, LoreKeeper, DevOps, y Designer.
...
*   **Resumen:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de directrices de agentes especializados (Personas) para el Reino.
*   **Cambios Clave:**
    *   **[Docs - Agentes]:** Se crearon 4 perfiles de contexto estandarizados dentro de `docs/agents/` que detallan las reglas, responsabilidades y prioridades para que cualquier agente de la arquitectura (Jarvis, Antigravity 2, etc.) asuma roles dedicados: Auditor de EconomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a/Seguridad, Depurador UI/Estado, Revisor de Calidad/Reglas y BotMaster de WhatsApp.

## 2026-05

### [2026-05-31] - [Autor: Antigravity]
*   **Archivos Modificados:** `GrimoireSection.tsx`, `MarketSection.tsx`, `imageUtils.ts` (Nuevo), `package.json`
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n Extrema (SWR CachÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â© y TransformaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de ImÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡genes).
*   **Cambios Clave:**
    *   **[Web] Performance (CachÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©):** Se reemplazÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el `useEffect` por `useSWR` en las llamadas pesadas de Supabase (Grimorio, Mercado, Bestiario, Flora) con un cachÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â© local de 5 minutos, logrando cargas instantÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡neas (0ms) al navegar entre pestaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±as.
    *   **[Web] Performance (ImÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡genes):** Se introdujo `getOptimizedImageUrl` para interceptar imÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡genes de Supabase Storage e inyectar el modo "render" para devolverlas comprimidas a formato WebP y tamaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±o miniatura.

### [2026-05-31] - [Autor: ui_ux_designer (Subagente) / Antigravity]
*   **Archivos Modificados:** MÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡s de 20 componentes React en `Kingdoom-sync/src` (ej. `AnimeHubSection.tsx`, `MarketItemCard.tsx`, etc.)
*   **Resumen:** OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n masiva de carga de imÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡genes en el frontend web.
*   **Cambios Clave:**
    *   **[Web] Performance:** Se inyectaron los atributos `loading="lazy"` y `decoding="async"` en todas las etiquetas `<img />` del proyecto para evitar cuellos de botella en la renderizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n y mejorar el tiempo de carga en listas pesadas como el mercado, el inventario y el anime hub.
...
*   **Cambios Clave:**
    *   **[Bot] Base de Datos:** Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ una violaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de la restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n `knowledge_documents_type_check`. El `type` del tracker se cambiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ de `tracker` a `other`, y se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el campo obligatorio `title`.

### [2026-05-30] - [Autor: Antigravity]
*   **Archivos Modificados:** `.gemini/antigravity/mcp_config.json` (Local IDE Config)
*   **Resumen:** ConfiguraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n e integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del servidor MCP de Vercel.
*   **Cambios Clave:**
    *   **[Tooling - MCP]:** Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ exitosamente el servidor MCP de Vercel (`https://mcp.vercel.com`) al entorno de Google IDE (Antigravity).
    *   **[Tooling - Auth]:** Se configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el Bearer Token de Vercel para permitir a los agentes realizar consultas de despliegues, logs de proyectos y administrar el entorno web alojado en Vercel sin salir del IDE.
...
*   **Resumen:** IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de "TavernScratchNative" y cierre definitivo de la Fase 3 de reactivaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n mobile.
*   **Cambios Clave:**
    *   **[Mobile - Minijuego]:** Se integrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ exitosamente el segundo minijuego nativo, "Rasca y Gana" (`TavernScratchNative.tsx`). Mantiene paridad con la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica web, empleando configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de probabilidades, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mites diarios y costos generados dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡micamente vÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a `getDailyScratchConfig`.
    *   **[Mobile - EconomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a Segura]:** La transacciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de oro utiliza exclusivamente `sessionStore.addGold`, el cual estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ respaldado por el RPC seguro de Supabase `increment_gold`, evitando cualquier condiciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de carrera.
    *   **[Mobile - Persistencia Local]:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `AsyncStorage` para manejar el lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mite de ganancias diarias de forma eficiente y segura a nivel de dispositivo.
    *   **[Mobile - IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n UI]:** El minijuego se renderiza de forma fluida y elegante en la tab del mercado, empleando la arquitectura `KingdoomUI` y `StaggerItem` existente.
    *   **[Backlog] Cierre Fase 3:** Con la implementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de este segundo minijuego y las notificaciones previamente aprobadas, la Fase 3 de reactivaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n mobile se considera cumplida.
*   **Notas/Advertencias:** Validado localmente con `npm run mobile:typecheck` sin errores nuevos atribuibles a esta funcionalidad.

### [2026-05-30] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/handlers/admin.js`, `AI_CHANGELOG.md`
*   **Resumen:** ReducciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del tiempo de gracia del comando `!purga` a peticiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del administrador.
*   **Cambios Clave:**
    *   **[Bot] Reglas de Purga:** Se modificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la duraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n permitida de un jugador sin ficha de 5 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as a 3 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as. Los textos de advertencia del menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âº de comandos tambiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©n fueron actualizados a 3 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as.

### [2026-05-30] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/tracker.js`, `kingdoom-bot/src/handlers/admin.js`, `AI_CHANGELOG.md`
*   **Resumen:** SoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n al reseteo del comando `!purga` provocado por reinicios del servidor en Hugging Face.
*   **Cambios Clave:**
    *   **[Bot] Persistencia en Supabase:** Se reescribiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `tracker.js` para que ya no guarde `pending_tracker.json` en el sistema de archivos local, ya que Hugging Face Spaces es efÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mero y borraba el progreso.
    ...
    *   **[Bot] Funciones AsÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­ncronas:** Se modificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `admin.js` para usar `await` en las llamadas del tracker, permitiendo consultas remotas.
...
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Notificaciones Mobile (Fase 3).
*   **Cambios Clave:**
    *   **[Mobile - Notificaciones]:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el servicio de notificaciones nativo (`notificationsService.ts`) que interactÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºa con Supabase para la app mobile, manteniendo paridad con la estructura de datos web.
    *   **[Mobile - Componente UI]:** Se creÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `PlayerNotificationBellNative.tsx` utilizando `TanStack Query` para polling cada 15 segundos y `react-native-reanimated` para animaciones fluidas del modal bottom-sheet. Se garantizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el tamaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±o de 46x46 en los elementos clickeables (incluyendo botÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de cerrar modal).
    *   **[Mobile - IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n]:** Se inyectÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la campana de notificaciones en el `rightSlot` de las pantallas principales `home.tsx` y `profile.tsx` para brindar mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡xima visibilidad sobre transacciones y recompensas sin afectar la ergonomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a ni requerir tabs adicionales.
    *   **[Backlog] ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Tareas de notificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n/features econÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³micas adicionales marcadas como evaluadas e implementadas.
*   **Notas/Advertencias:** Validado con `npm run mobile:typecheck` limpio. La economÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³vil sigue intacta ya que la campana es solo un observador de estado.
...
*   **Resumen:** Segunda pasada de polish visual/ergonÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³mico para la reactivaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n mobile (Fase 3). 
*   **Cambios Clave:**
    *   **[Mobile - UI/ErgonomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a]:** Se estandarizaron los touch targets a un mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­nimo de 46px en componentes interactivos clave de toda la app (botones, tabs, pills y search inputs) para cumplir con las guÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as de accesibilidad en Android. Esto incluyÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ ajustes en Archivist, Anime, Exchange, HorseRace y Slots, solucionando los problemas de fat-finger.
    *   **[Arquitectura - DecisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n]:** Se evaluÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ si `Archivist` y `Anime` justifican una expansiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n profunda. Se decide mantenerlos como mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³dulos compactos. `Archivist` sirve como referencia rÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡pida y `Anime` como un hub de enlaces ligeros. AÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adirles una navegaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n profunda y dependencias pesadas impactarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a negativamente el rendimiento de React Native y la filosofÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a "compact-mode" que guÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a el resurgimiento mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³vil.
*   **Notas/Advertencias:** Todos los cambios pasaron `npm run mobile:typecheck` y no se inyectÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica econÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³mica nueva.
...
*   **Resumen:** Cierre definitivo de Fase 2 (Antigravity 2) de reactivaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n mobile: sincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n atÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³mica de economÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a y estabilizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del Stock Exchange.
*   **Cambios Clave:**
    *   **[Mobile - EconomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a] QA Funcional Completo y SincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n atÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³mica:** Se auditÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ funcionalmente el ciclo de compra en el mercado mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³vil (se descuenta una sola vez, se refresca el saldo, se actualiza el inventario). Se refactorizaron `TavernSlotsNative`, `TavernHorseRaceNative` y `RealmStockExchangeNative` para utilizar `addGold` (basado en el RPC `increment_gold` de Supabase) en lugar del mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©todo inseguro `updateGold`, previniendo race conditions y asegurando la consistencia entre saldo visible e historial. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡s, se documentÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el SQL del RPC `increment_gold` (`supabase_increment_gold.sql`) en el repositorio para evitar desincronizaciones futuras. Fase 2 completada y validada con \`npm run mobile:typecheck\` y \`npm run build\`.
    *   **[Mobile - Mercado] EstabilizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Se verificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la paridad operativa del `RealmStockExchangeNative` con la web, confirmando que las operaciones respetan el bloqueo transaccional (`applyOperation`) y propagan correctamente los deltas de oro.
    *   **[Backlog] ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Se marcaron como completadas las tareas restantes del sprint de Antigravity 2 en `mobile-reactivation-backlog.md`.
*   **Notas/Advertencias:** Validado con lectura de cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³digo y `npm run mobile:typecheck` exitoso. La arquitectura financiera mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³vil ya no sobrescribe el oro total, operando exclusivamente mediante incrementos/decrementos validados en Supabase.
...
*   **Cambios Clave:**
    ...
    *   **[Arquitectura] Matriz de paridad:** Se documento `web vs mobile` por dominio con estado (`lista`, `parcial`, `ausente`, `no prioritaria`) y dueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±o principal.
...
### [2026-05-29] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/components/TavernPlinko.tsx`, `AI_CHANGELOG.md`
*   **Resumen:** AmpliaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de la cantidad mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡xima de esferas a lanzar en la Torre del Mago.
*   **Cambios Clave:**
    *   **[Minijuegos - Torre del Mago]:** Se ajustaron los botones de selecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de cantidad de esferas, cambiando las opciones de `[1, 3, 5, 10]` a `[1, 5, 10, 20]`.
*   **Notas/Advertencias:** ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n rÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡pida de UI para escalar las apuestas.
...
*   **Resumen:** ReducciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del plazo de inactividad para purga de 5 a 3 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as.
*   **Cambios Clave:**
    *   **[Admin] Comando !purga:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la constante `THREE_DAYS_MS` y la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica de cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo de tiempo para que el bot advierta y expulse a los usuarios sin ficha o inactivos luego de 3 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as en lugar de 5.
    *   **[Admin] Notificaciones de UI:** Se ajustaron los textos enviados por WhatsApp al ejecutar la purga para que reporten correctamente el lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mite de 3 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as.
...
*   **Resumen:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n visual y lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica del cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo de apuestas en Esfera de las Runas.
*   **Cambios Clave:**
    *   **[Minijuegos] CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo de total:** El costo total de la jugada ahora calcula `apuestaPorEsfera * cantidad`, mostrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­citamente y utilizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose correctamente para la deducciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de oro y los cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lculos de RTP/premio.
    *   **[UI] Claridad de Etiquetas:** Actualizadas etiquetas "Multiplicador" a "Lanzamiento" y "Apuesta unitaria" a "Apuesta por esfera" para evitar confusiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n. Se muestra de manera clara el costo total real de la jugada.
    *   **[UX] Estado del BotÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Actualizados los mensajes en el botÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de lanzar (Oro insuficiente, lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mite alcanzado, apuesta invÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lida) para informar dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡micamente y con base al oro que requiere la apuesta total.
...
*   **Cambios Clave:**
    *   **[Admin] BotÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n Eliminar:** Nuevo botÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de "Eliminar" en la tarjeta de participante (`AdminMissionManager.tsx`) con prompt de confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de seguridad.
    *   **[Backend] Borrado y RecÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡lculo:** La funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n `deleteMissionClaim` (`missions.ts`) borra el reclamo, elimina las pruebas del Storage (`MISSION_EVIDENCE_BUCKET`) y devuelve el estado de la misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n a `available` automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente si se libera un cupo en una misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n `in-progress`.
*   **Notas/Advertencias:** Listo para producciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n, confirmado con `git push`. Las validaciones de TS fallan por errores previos ajenos al ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡rea afectada.
...
*   **Resumen:** SincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n en tiempo real de misiones en UI, resoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de conflictos y limpieza de repositorio.
*   **Cambios Clave:**
    *   **[Mobile/UI] SincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de misiones:** Implementada lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica para reflejar cambios de estado de misiones (reclamos de cupos, actualizaciones de admin) en tiempo real en la UI mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³vil sin recarga manual.
    *   **[Mantenimiento] ResoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de conflictos:** Solucionados conflictos de Git en `AdminControlSheet.tsx` y `AdminControlPrimitives.tsx` para mantener consistencia Mobile-First.
    *   **[Mantenimiento] Limpieza de metadatos:** Se auditaron y limpiaron residuos huÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©rfanos de Git (`REBASE_HEAD`, `.COMMIT_EDITMSG.swp`) tras confirmar un working tree limpio.
    *   **[DocumentaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n] CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Schema:** Actualizado `DATABASE_SCHEMA.md` para clarificar la diferencia de casing entre `playerId` (`character_sheets`) y `player_id` (`player_inventory`).
    *   **[Mantenimiento] Archivos temporales:** Eliminados scripts temporales (`patch.cjs`, `test-supabase.ts`) que no pertenecen a producciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n.
*   **Notas/Advertencias:** Persisten errores de tipos de Typescript en `RankingCard.tsx` y `WeeklyRankingPodium.tsx` que requieren futura revisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n.
...
*   **Resumen:** Puente canÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³nico entre misiones del panel admin y el Game Master para restringir magias de NPCs al grimorio oficial.
*   **Cambios Clave:**
    *   **[Admin] NPCs tÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡cticos canÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³nicos:** El editor de misiones ahora permite definir NPCs del encounter con rol, stats, notas tÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡cticas y una lista explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­cita de magias permitidas tomadas del grimorio administrado.
    *   **[Compatibilidad] Config embebida sin migraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** La configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del GM se serializa dentro de `instructions` usando bloques `[GM_CONFIG]...[/GM_CONFIG]`, evitando cambios de esquema en Supabase y manteniendo compatibilidad con las misiones existentes.
    *   **[GM-bot] Magia restringida por payload:** El bot parsea esa configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n embebida y la inyecta en `DATOS_DE_MISION` como bloque canÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³nico de NPCs y magias permitidas, junto con una regla explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­cita para no inventar hechizos fuera de la lista.
*   **Notas/Advertencias:** `npx tsc --noEmit` sigue fallando por errores previos y ajenos en `src/components/RankingCard.tsx` y `src/components/WeeklyRankingPodium.tsx`. La validaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de estos cambios se hizo con chequeo sintÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ctico del bot y revisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n focalizada del flujo admin -> misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n -> GM.
...
*   **Resumen:** OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del motor del Game Master para rol narrativo orgÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡nico sin lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mites rÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­gidos y agnÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³stico al lore.
*   **Cambios Clave:**
    *   **Prompt DinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡mico y AgnÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³stico:** Se refactorizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `buildGMPrompt` en `gmTracker.js` para eliminar referencias estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ticas (como "Shadow Garden"). Ahora el GM adopta la personalidad y el lore definidos exclusivamente en las instrucciones de la misiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n desde la base de datos.
    *   **EliminaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de LÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mites y Formato Natural:** Se removiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de 350 palabras y el uso de listas numeradas (1., 2., 3...). El bot ahora usa prosa fluida y bloques de cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³digo Markdown para exponer mecÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡nicas RPG (cooldowns, niveles, daÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±o) imitando el estilo de rol avanzado humano.
    *   **ExpansiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Tokens:** Se incrementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ `maxOutputTokens` de 1024 a 2048 en `ai.js` para prevenir que respuestas narrativas extensas se corten prematuramente.
    *   **Fidelidad TÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ctica:** El sistema ahora estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ instruido para priorizar el respeto estricto a las estadÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­sticas reales (niveles, HP, etc.) de los NPCs creados en el panel de control.
...
*   **Resumen:** RevisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n integral de UX/UI Mobile-First para compactar y optimizar espacio en pantallas pequeÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±as.
*   **Cambios Clave:**
    *   **[UI Admin] Modal Full-screen:** `AdminControlSheet` ahora ocupa el 100% de la pantalla en dispositivos mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³viles sin bordes redondeados, maximizando el espacio ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºtil, mientras que en desktop mantiene su diseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±o de panel flotante (`md:h-[92vh] md:rounded-[2rem]`).
    ...
    *   **[UI Perfil] OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Layout:** `PlayerProfilePanel` ajustÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la separaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de sus bloques (`gap-5` a `gap-4 sm:gap-5`) y compactÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ los paddings generales de sus secciones internas para eliminar espacios vacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­os innecesarios sin perder jerarquÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a visual.
*   **Notas/Advertencias:** NingÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºn cambio de lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³gica de Supabase ni del bot. Exclusivo de Frontend UI.

### [2026-05-26] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/auditLog.js`, `kingdoom-bot/src/adminStore.js`
*   **Resumen:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de rutas absolutas para garantizar persistencia local y remota del bot.
*   **Cambios Clave:**
    *   **[Admin] Rutas dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡micas:** Se implementaron rutas dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡micas (usando `__dirname` y `path.join`) para `admin_audit_log.json` y `admins.json`. Esto corrige el fallo silencioso donde el comando `!bitacora` no mostraba informaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n al correr en Windows y asegura compatibilidad nativa tanto local como en el contenedor de Hugging Face.

### [2026-05-25] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/handlers/player.js`
*   **Resumen:** ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del comando `!ayuda`.
*   **Cambios Clave:**
    *   **[Admin/Soberano] MenÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âº de Ayuda:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adieron los comandos administrativos faltantes (`!actividad`, `!grupoactual` y `!groupid`) a la lista desplegada por el comando `!ayuda`.

### [2026-05-25] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/handlers/admin.js`, `kingdoom-bot/src/index.js`, `kingdoom-bot/src/supabase.js`
*   **Resumen:** CreaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n del comando de reporte `!actividad` (o `!inactivos`).
*   **Cambios Clave:**
    *   **[Admin] Reporte de Inactividad:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!actividad` exclusivo para administradores, el cual extrae a todos los usuarios ordenados por su ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºltima fecha de conexiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n y los formatea visualmente en columnas monospaciadas para rÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡pida lectura en WhatsApp.
...
*   **Resumen:** Sistema de purga automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡tica por 15 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as de inactividad (Web y WhatsApp).
*   **Cambios Clave:**
    *   **[Base de Datos] SQL Cron:** Nuevo script para aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adir la columna `last_active_at` y crear un cron diario (`pg_cron`) que purgue perfiles inactivos.
    *   **[Web] Rastreo de Actividad:** Se ha integrado `touchPlayerActivity` al iniciar o recuperar sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n en la web para evitar purgas errÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³neas.
    *   **[Bot] IntercepciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de Mensajes:** Todo comando procesado por el bot en WhatsApp actualizarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ la actividad del usuario en tiempo real.

### [2026-05-25] - [Autor: Antigravity]
*   **Archivos Modificados:** `kingdoom-bot/src/handlers/admin.js`
*   **Resumen:** Mejora del comando !purga para reportar y etiquetar a los usuarios pendientes.
*   **Cambios Clave:**
    *   **[Admin] Reporte de dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as restantes:** El comando `!purga` ahora enumera a todos los usuarios pendientes que aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºn no han superado el lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­mite de 5 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as, mencionÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ndolos mediante etiqueta (`@usuario`) y mostrando cuÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ntos dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as les quedan para ser eliminados ("X dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­as para eliminaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n"). Esto funciona en adiciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n a la expulsiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡tica de aquellos que ya hayan cumplido el plazo.

### [2026-05-25] - [Autor: Antigravity]
*   **Archivos Modificados:** `AI_CHANGELOG.md`, `ai-memory/kingdoom-memory.jsonl`, `mcp_config.json`
*   **Resumen:** Reforzamiento de reglas de protocolo e integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n local del MCP Kingdoom-memory.
*   **Cambios Clave:**
    *   **[Core Rule] Registro Obligatorio:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la regla de Inteligencias Artificiales del changelog para exigir que **cualquier** cambio, por mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­nimo que sea, deba documentarse en el historial y en la memoria MCP, y subirse obligatoriamente a Git de inmediato.
    *   **[Core Rule] SincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Se inyectaron directrices principales (`core-rule`) en la memoria MCP exigiendo sincronizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n obligatoria inicial (`git pull`) y publicaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n obligatoria final (`git push`) en cada sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n.
    *   **[Sistema] Servidor MCP:** Se configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ exitosamente el servidor local MCP en `mcp_config.json` para tener acceso nativo a la memoria compartida de la IA.
...
*   **Resumen:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de bug de usuarios con mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºltiples nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmeros en reportes, implementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de mensajes motivacionales automatizados, y habilitaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n oficial de sistema multicuentas para WhatsApp.
*   **Cambios Clave:**
    *   **Bugfix en !pendientes y Scheduler (Fix):** Se ajustÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n de limpieza de nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmeros `normalizePhone` porque estaba fusionando nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmeros separados por coma en un ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºnico nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmero corrupto. Ahora, cuando el bot revisa listas de participantes o envÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a notificaciones masivas, separa las comas primero y evalÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºa cada nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmero individualmente, arreglando falsos positivos de "no registrados" para los administradores y permitiendo que les lleguen las recompensas.
    *   **Mensajes Motivacionales de Rol (Feature):** El planificador de tareas (`scheduler.js`) fue rediseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±ado. Se eliminÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ el reporte semanal de ranking, y ahora envÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a un mensaje inmersivo y poÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©tico ("Un nuevo ciclo comienza...") todos los lunes. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡s, el aviso de reset diario a la medianoche fue adaptado para incluir el nombre del personaje principal del usuario, haciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©ndolo 100% de rol.
    *   **Soporte Multicuentas (Feature):** Se eliminÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ la restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n en la base de datos que impedÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â­a a los usuarios vincular un nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmero de telÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©fono que ya estaba en uso. 
    *   **Comando `!cambiarcuenta` (Nuevo):** Los jugadores con mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºltiples cuentas web (ej: Nothing y Alexander) pueden vincular ambas a su mismo WhatsApp. Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³ un store local (`activeProfileStore.js`) y un comando `!cambiarcuenta <nombre>` para que el jugador elija cuÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡l de sus fichas estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡ activa para interactuar con el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡culo, jugar o recibir oro.

### [2026-05-25] - Oracle and Profile Improvements
- Conectado el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¡culo a las tablas de eventos y misiones activas.
- Refinado el comando '!perfil' para separar y clasificar inteligentemente los IDs de WhatsApp y los nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âºmeros de telÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â©fono reales.
- AÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â±adidos comandos faltantes al menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Âº de ayuda (!ayuda) con restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â³n por roles.
...
*   **Resumen:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de parseo en comandos y expansiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de la visiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo hacia el Inventario Real.
*   **Cambios Clave:**
    *   **Trim de prefijo (Fix):** Se ajustÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `parseCommand` en `player.js` para aplicar un `.trim()` sobre el string inmediatamente despuÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©s de remover el prefijo `!`. Esto soluciona un error crÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­tico donde comandos como `! Verificar <id>` se registraban como comando vacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­o (`""`) debido al espacio residual.
    *   **Inventario en el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo (Feature):** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `getPlayerInventory` en `supabase.js` para consultar la tabla `player_inventory`. Ahora el `handleOraculo` en `games.js` inyecta las compras reales del mercado web (con sus cantidades correspondientes) directo al contexto de la IA. Si el jugador le pregunta "ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â¿Ã‚Â½Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¥Ã‚Â Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¿CÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³mo es mi equipamiento?", el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo ya no alucinarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ basÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose solo en su ficha original, sino que comentarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡gicamente sobre las pociones o espadas reales que haya adquirido con oro.
...
*   **Resumen:** Mejora del OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo con Memoria y Contexto de Jugador.
*   **Cambios Clave:**
    *   `games.js`: Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un mapa en memoria (`oraculoMemory`) que guarda el historial de los ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºltimos 3 intercambios por cada chat/grupo, dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndole al OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo memoria a corto plazo.
    *   El orÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo ahora sabe quiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n le habla y cuÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡nto oro tiene. El prompt fue ajustado para referirse al jugador por su nombre, y para burlarse o codiciar sus riquezas basÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose en su saldo en la base de datos, mejorando drÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡sticamente el rol en vivo.
    *   **Flexibilidad (Nuevo):** Se eliminÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n rÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­gida de "exactamente 2-3 lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­neas". Ahora se le permite adaptarse: puede dar respuestas de 1-2 lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­neas si es un simple vaticinio o explayarse hasta 2 pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡rrafos si la pregunta requiere contexto del *lore*. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s, puede interpretar preguntas "Off-Rol" (fuera de personaje) absorbiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©ndolas de forma poÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©tica como si fueran hechicerÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a o idiomas forasteros.
    *   **IntegraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Fichas (Nuevo):** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `getPlayerSheet` en `supabase.js`. El orÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo ahora extrae la Ficha de Personaje (Rol) del jugador desde Supabase e inyecta su Nombre de personaje, Raza, Origen, Poderes, Arma y Personalidad en el sistema de la IA. Esto permite al orÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo dar profecÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­as hiper-personalizadas basadas en la lore individual de cada guerrero.
    *   `ai.js`: Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un tercer modelo de respaldo (`gemini-1.5-flash`) en la cascada de fallbacks para mitigar errores `503 Service Unavailable` provocados por la saturaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n global de los servidores de Google Generative AI en los modelos `2.5` y `3.5`.
    *   `ai.js`: Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un tercer modelo de respaldo (`gemini-1.5-flash`) en la cascada de fallbacks para mitigar errores `503 Service Unavailable` provocados por la saturaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n global de los servidores de Google Generative AI en los modelos `2.5` y `3.5`.
    *   **PrevenciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Alucinaciones (Nuevo):** Se le dio la instrucciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n estricta al OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo de negarse a revelar las riquezas o secretos de *otros* jugadores. Si se le pregunta por alguien ajeno, ahora dirÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ de forma misteriosa que no puede revelar secretos que estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡n bajo la sombra, evitando que la IA invente nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmeros falsos para compensar la falta de contexto en memoria.
    *   **Transferencia de Oro (`!oro`):** Se modificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!oro` en `player.js`. Ahora, si se usa sin parÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡metros, muestra el saldo actual. Si se usa como `!oro <monto> <@usuario>`, permite a los jugadores enviarse oro entre sÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­, descontando de la cuenta del emisor y sumando a la del receptor (con validaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de fondos y protecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de auto-envÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­o).
...
*   **Resumen:** Arquitectura RAG e integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Base de Conocimiento entre Kingdoom-sync (Archivista) y Kingdoom-bot.
*   **Cambios Clave:**
    *   `supabase.js`: Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adieron funciones `getKnowledgeDocuments` y `pickKnowledgeContext` para consultar la tabla `knowledge_documents`.
    *   `!oraculo` (`games.js`): Ahora inyecta dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡micamente hasta 2 documentos relevantes de la base de datos de conocimiento como contexto al prompt de Gemini, compartiendo la misma memoria del Archivista web.
    *   `!data` (`admin.js` y `index.js`): Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ este comando exclusivo de admin para WhatsApp. Permite adjuntar un archivo `.txt` y cargarlo a la tabla Supabase, sincronizando la memoria directamente desde WhatsApp hacia la web.

### [2026-05-22] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/components/AdminControlSheet.tsx`, `src/features/businesses/businesses.service.ts`
*   **Resumen:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de la funcionalidad de borrado de negocios y propuestas de negocios desde el panel de control administrativo.
*   **Cambios Clave:**
    *   **[Backend] EliminaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de registros:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adieron las funciones `deleteBusiness` y `deleteBusinessProposal` a los servicios de negocios para ejecutar los borrados con su respectivo manejo de estado.
    *   **[Admin] BotÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n Borrar Negocio Activo:** Los administradores ahora pueden borrar negocios permanentemente pulsando el ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­cono de la papelera junto al estado del almacenamiento en la tarjeta del negocio, con un diÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡logo de confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n previo.
    *   **[Admin] BotÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n Borrar Propuesta:** Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un botÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n rojo de "Borrar" en el formulario de creaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n/ediciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de propuestas, posibilitando la eliminaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de propuestas mal formuladas o expiradas, igualmente protegido por confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n.
*   **Notas/Advertencias:** Estas acciones no se pueden deshacer y el oro no reclamado en negocios activos se perderÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ si son eliminados.

### [2026-05-22] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/components/PlayerProfilePanel.tsx`
*   **Resumen:** OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de la interfaz de "Tus negocios" para ahorrar espacio y mejorar la experiencia de usuario.
*   **Cambios Clave:**
    *   **[UI] Filtrado AutomÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tico:** Las propuestas de negocios ahora desaparecen instantÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡neamente de la lista "Propuestas pendientes" una vez que son respondidas, mostrando solo aquellas en estado "pending".
    *   **[UI] SecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n Colapsable:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un botÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n "Mostrar / Ocultar" en la cabecera. Por defecto, todo el bloque interno de "Negocios activos" y "Propuestas" aparece colapsado, limpiando visualmente el perfil del jugador.
*   **Notas/Advertencias:** Estos cambios operan exclusivamente a nivel de presentaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n en la SPA; la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³gica de red y base de datos (RPC) permanece intacta.
...
### [2026-05-20] - [Autor: Antigravity] - [SesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n 3 - AuditorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a de Comandos]
*   **Archivos Modificados:** `kingdoom-bot/src/handlers/games.js`, `kingdoom-bot/src/handlers/admin.js`, `kingdoom-bot/src/index.js`
*   **Resumen:** AuditorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tica de todos los handlers del bot y correcciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de permisos/comandos de administraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n.
*   **Cambios Clave:**
    *   **[SOPORTE] Comando `!pendiente` singular:** Se mapeÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `!pendiente` en el router principal (`index.js`) y en `admin.js` para que los administradores y dueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±os puedan usar tanto el formato singular como el plural (`!pendientes`). Anteriormente, usar el singular hacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a que la peticiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n fuera procesada por la IA al no estar en la lista blanca de comandos de administrador en `index.js`.
    *   **[SOPORTE] Acceso a `!censo` y `!pendientes` para Administradores:** Se validÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ y asegurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que los usuarios que posean privilegios de administrador (ademÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s del Owner) puedan ejecutar `!censo` y `!pendientes` sin restricciones de permisos.
    *   **[CRÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã…Â½Ã‚Â§ICO] Fix `!dados` ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¢ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾ÃƒÂ£Ã¯Â¿Â½Ã…Â½ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¯Ã‚Â¿Ã‚Â½ sender incorrecto en grupos (`games.js`):** El comando `!dados` usaba `msg.from` para buscar al jugador en Supabase. En grupos de WhatsApp, `msg.from` devuelve el JID del **grupo** (ej: `12345@g.us`), no el del jugador. Esto hacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a que el bot nunca encontrara al jugador y siempre respondiera "No estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s registrado". Corregido usando `msg.author || msg.from`, el patrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndar del resto de los handlers.
    *   **[MEDIO] Fix `!ban` ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¢ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾ÃƒÂ£Ã¯Â¿Â½Ã…Â½ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¯Ã‚Â¿Ã‚Â½ falso positivo (`admin.js`):** Cuando un admin ejecutaba `!ban` con un nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero no registrado en la DB, Supabase actualizaba 0 filas sin lanzar un error, y el bot respondÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a "baneado" falsamente. Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una verificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n previa que consulta al jugador y retorna un error claro si no existe. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s, el mensaje de confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n ahora muestra el **username** del jugador baneado, no solo el nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero.
    *   **[MEJORA] Comando `!grant` y nuevo `!quitar` (`admin.js`, `index.js`):** Se mejorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la gestiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de oro para los administradores. Ahora `!grant` acepta tanto el celular, el **nombre de usuario**, o el **ID de la pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡gina web** (prefijo UUID) del jugador (ej. `!grant Zoelfrost 1000`, `!grant 2354 1000`), facilitando enormemente la administraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s, se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!quitar` para restar oro sin necesidad de usar nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmeros negativos (ej. `!quitar Zoelfrost 500`). Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âº de ayuda (`!admin`) para reflejar estos cambios.
    *   **[MEJORA] OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo y Memoria**
        - **InyecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Inventario Real:** El OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo ahora lee el inventario real del jugador (comprado en el mercado con oro) y lo integra en sus respuestas. Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un error en la consulta a Supabase que causaba fallos silenciosos al buscar la columna `category` (que en realidad es `item_category`), logrando que el bot vuelva a "ver" los ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­tems correctamente, extrayendo tambiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n el `item_name`. AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s, se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una inyecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­cita para inventarios vacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­os, evitando que el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo "evada" la pregunta con frases mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­sticas cuando el jugador no tiene ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­tems.
        - **Identidad del Jugador (15-digit ID Fix):** Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un mapeo interno para que el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo reconozca correctamente el ID de 15 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­gitos (`275162062668001`) del Owner como el perfil principal (`595987273405`), evitando que el sistema lo trate como un "alma sin nombre".
        - **Personalidad Mejorada:** Se rediseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el prompt del OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo para que actÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºe como un "vidente veterano y cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­nico", hablando de forma mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s directa, coloquial y menos poÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©tica. Su longitud se limitÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ a 3 pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡rrafos y se instruyÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ para negarse a revelar fortunas de terceros.
    *   **[MEJORA] Baneo y gestiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Administradores unificada (`admin.js`):** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un helper centralizado para que `!ban`, `!add admin`, `!remove admin`, `!grant` y `!quitar` puedan procesar a los jugadores usando su **ID web**, **username** o **celular**. Esto estandariza la experiencia de administraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n, permitiendo identificar jugadores de mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºltiples maneras, tal como se hace en el comando de vinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `!verificar`.
    *   **[ELIMINADO] Comando `!broadcast` removido (`admin.js`, `index.js`):** El comando fue eliminado por decisiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del Soberano. WhatsApp ya ofrece la funcionalidad nativa de @all / @todos en grupos, lo que hace innecesario un broadcast por DM que ademÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s tenÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a problemas de compatibilidad con nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmeros no registrados.
    *   **[NUEVO] Comando `!purga` (`admin.js`, `tracker.js`, `index.js`):** Nuevo comando que permite al Staff expulsar del grupo de WhatsApp a los usuarios que llevan mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s de 5 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­as sin hacer su ficha. El bot mantiene un archivo JSON interno (`pending_tracker.json`) que registra la primera vez que un usuario aparece en `!pendientes`. Al ejecutar `!purga`, el bot verifica quiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©nes superaron los 5 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­as y los remueve automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente. Requiere que el bot sea admin del grupo.
    *   **[MEJORA] `!pendientes` ahora rastrea fechas (`admin.js`, `tracker.js`):** Cada vez que se ejecuta `!pendientes`, el bot registra la fecha de detecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de cada usuario pendiente. Esto alimenta al tracker que `!purga` consume para calcular los 5 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­as de gracia.
    *   **[FIX] `!censo` / `!fichas` ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¢ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾ÃƒÂ£Ã¯Â¿Â½Ã…Â½ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¯Ã‚Â¿Ã‚Â½ columna inexistente (`supabase.js`):** La query de `getRealmCensus()` pedÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a `player_id` a la tabla `character_sheets`, pero esa columna no existe en Supabase (solo existe `playerId` en camelCase). Esto hacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a que el comando fallara con "Error al obtener el censo del reino". Corregido removiendo la columna fantasma.
    *   **[BONUS] Formato de oro en `!dados`:** Se aplicÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `.toLocaleString('es-PY')` al mostrar el oro del jugador en el mensaje de saldo insuficiente, siendo consistente con el resto del bot.

### [2026-05-20] - [Autor: Antigravity] - [SesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n 3 - Fix OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo Cuota y MigraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n a Gemini 2.5]
*   **Archivos Modificados:** `kingdoom-bot/src/ai.js`
*   **Cambios Clave:**
    *   **[CRÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã…Â½Ã‚Â§ICO] Fallback de modelo en `!oraculo` (`ai.js`):** Se identificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que todos los modelos Gemini 1.0 y 1.5 (incluyendo `gemini-1.5-flash`) fueron desactivados por Google, arrojando error `404 Not Found`. Se migrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el modelo por defecto del bot de `gemini-1.5-flash` a **`gemini-2.5-flash`**.
    *   **[CRÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã…Â½Ã‚Â§ICO] Soporte para mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºltiples claves API con rotaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tica (`ai.js`):** El usuario configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ dos llaves API separadas por comas en `GEMINI_API_KEY`. Se rediseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el manejador para procesar una lista de llaves de manera dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mica. Al invocar el OrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡culo, intenta secuencialmente con cada clave. Si una falla (por ejemplo, por lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­mite de cuota o error 429), realiza un log detallado y reintenta con la siguiente clave transparente y automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente.
    *   **[MEJORA] Cadena de Fallback de Modelos en caso de 404/503 (`ai.js`):** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³gica de fallback de modelos en bucle. Si el modelo actual (ej: `gemini-2.5-flash`) devuelve `404 Not Found` o un error temporal de sobrecarga `503 Service Unavailable`, el bot no descartarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ la clave de inmediato; en su lugar, intentarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente con otros modelos candidatos como **`gemini-3.5-flash`** para asegurar respuestas exitosas durante picos de demanda del servidor de Google.

### [2026-05-20] - [Autor: Antigravity] - [SesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n 3 - AuditorÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a Scheduler]
*   **Archivos Modificados:** `kingdoom-bot/src/scheduler.js`
*   **Cambios Clave:**
    *   **[CRÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã…Â½Ã‚Â§ICO] Fix reset semanal `weekly_gold` (`scheduler.js`):** La operaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `supabase.from('players').update({ weekly_gold: 0 })` sin ningÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºn filtro es **bloqueada por defecto** por Supabase JS v2 como medida de seguridad contra actualizaciones masivas accidentales. Esto hacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a que el ranking semanal se anunciara correctamente cada lunes pero el oro semanal nunca se reseteara, acumulÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose indefinidamente. Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ agregando `.gte('weekly_gold', 0)` como filtro de seguridad que coincide con todos los jugadores (el oro nunca es negativo por diseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o).
...
*   **Resumen de Tareas:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del sistema de bienvenida, comando `!groupid`, fix del mercado, correcciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de textos truncados, fix de imports en consultas detalladas y migraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del bot a Hugging Face Spaces (16 GB RAM gratis).
*   **Cambios Clave:**
    *   **MigraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n a Hugging Face Spaces:** Se trasladÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el bot desde Railway (con crÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©ditos agotados) a Hugging Face Spaces basado en Docker, obteniendo **16 GB de RAM y 2 vCPU** de forma completamente gratuita, eliminando crasheos de memoria por Puppeteer/Chromium.
    *   **ResoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de puertos (7860) y metadatos:** Se agregÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `ENV PORT=7860` en el `Dockerfile` y se creÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el `README.md` con la cabecera YAML requerida por Hugging Face. Esto solucionÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la pantalla infinita de "Preparing Space" permitiendo la comunicaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n correcta con la interfaz web.
    *   **Fix de permisos no-root:** Se crearon los directorios del bot y se asignÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `chmod -R 777` en el Dockerfile para que el usuario de Hugging Face (`1000`) pueda escribir los datos de autenticaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de WhatsApp en la carpeta temporal de persistencia.
    *   **Fix del comando !mercado (columna 'available' inexistente):** Se detectÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que las consultas a la tabla `market_items` en `supabase.js` filtraban usando `.eq('available', true)`. Dado que la columna `available` no existe en la base de datos de Kingdoom (el stock se gestiona en su lugar con `stock_status`), la API de Supabase devolvÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a un error de columna inexistente, causando que el bot reportara falsamente que el mercado estaba vacÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­o. Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ removiendo este filtro y adaptando `getRealmSnapshot` para excluir items con `stock_status = 'sold-out'`.
    *   **Fix de importaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de getMissionDetails y getEventDetails:** Los comandos de detalle de misiones (`!mision <nombre>`) y eventos (`!evento <nombre>`) fallaban silenciosamente lanzando el error de sistema `"El reino estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ en llamas..."`. Se detectÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que las funciones `getMissionDetails` y `getEventDetails` no estaban importadas al inicio de `player.js` desde `../supabase.js` a pesar de estar declaradas e implementadas. Se agregaron a los imports del archivo para solucionar el fallo de referencia.
    *   **AmpliaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­mite de texto en comandos (!item, !mision, !evento):** Las descripciones y habilidades se recortaban excesivamente en WhatsApp (`clipText` recortaba a 110, 130 o 140 caracteres, dejando textos incompletos con suspensivos). Se ampliÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­mite en los comandos de detalle a **500 caracteres**, permitiendo la lectura de habilidades legendarias completas y descripciones extendidas sin spam descontrolado.
    ...
    *   **Log de diagnÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³stico:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ade `console.log` para `group_join` detallando los IDs de grupos.
    *   **Comando !groupid:** Se creÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!groupid` para administradores que devuelve el JID ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºnico del grupo (`@g.us`) donde se ejecuta para poder configurar las variables del bot de bienvenida.
*   **Notas/Advertencias:** El bot estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ completamente enlazado, conectado y activo de forma gratuita en su nueva infraestructura de Hugging Face Spaces.

### [2026-05-19] - [Autor: Antigravity] - [SesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n 2]
...
*   **Resumen de Tareas:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del sistema de vinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n segura (`!verificar`) entre perfiles web y WhatsApp y visualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n en el Panel de Perfil de la web.
*   **Cambios Clave:**
    *   **Comando de VinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n !verificar:** Se creÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `verifyAndLinkPlayer` en el backend del bot (`kingdoom-bot/src/supabase.js`) que permite a cualquier usuario vincular su nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero de WhatsApp con su cuenta web medieval existente ingresando su nombre de usuario (sin distinguir mayÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsculas/minÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsculas) o el segmento inicial de su ID UUID (ej. `!verificar Zoelfrost` o `!verificar 2354`).
    *   **Bypass de Jugador no Registrado:** Se ubicÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el manejador de `!verificar` en `kingdoom-bot/src/handlers/player.js` arriba del control de seguridad de usuario no registrado, permitiendo que nuevos contactos puedan vincularse de manera fluida sin ser rechazados como viajero desconocido.
    *   **VisualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de ID en la Web:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `src/components/PlayerProfilePanel.tsx` tanto en la vista colapsada como expandida para mostrar el ID corto (los primeros 8 caracteres del UUID) de manera clara y estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©tica.
    *   **Instrucciones de VinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n en UI:** En caso de que la cuenta web no tenga ningÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºn WhatsApp vinculado (`player.phone` es null), el Panel de Perfil muestra una tarjeta dorada estilizada con instrucciones precisas y el comando exacto para copiar y enviar al bot: `!verificar <id_corto>`.
    *   **ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Modelos y Consultas:** Se incluyÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la columna `phone` en todas las consultas y payloads de creaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de jugadores de `src/utils/players.ts` para que el estado de vinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n se sincronice en tiempo real con la UI de la SPA.
    *   **HabilitaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del Comando en Ruteador:** Se registrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `'verificar'` en la lista blanca de comandos del ruteador principal `kingdoom-bot/src/index.js` para asegurar el procesamiento correcto de su prefijo.
    *   **Soporte de Citado para !add/!remove admin:** Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una discrepancia UX donde los comandos `!add admin` y `!remove admin` requerÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­an especificar manualmente el nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero. Ahora soportan plenamente citar (responder a) un mensaje para extraer automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente el nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero del remitente del mensaje citado (OpciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n A).
    *   **PreservaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Prototipo de Mensaje:** Se solventÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un error crÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­tico de `TypeError: msg.getQuotedMessage is not a function` que provocaba que el bot crasheara con "El reino estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ en llamas..." al usar citados en comandos modificados. La causa era que la destructuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `{ ...msg }` eliminaba los mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©todos de la clase `Message` de `whatsapp-web.js`. Se solucionÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ implementando un envoltorio limpio basado en `Object.create(originalMsg)` que preserva la cadena de prototipos intacta.
    *   **Administradores Persistentes en Supabase:** Se detectÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que el almacenamiento local `admins.json` dentro del contenedor de Railway se perdÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a al redesplegar la aplicaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n. Para solucionar esto de forma definitiva, se habilitÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el chequeo hÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­brido: el bot ahora valida los privilegios de administrador consultando la columna `is_admin` en la tabla `players` de Supabase de manera asÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­ncrona. Los comandos `!add admin` y `!remove admin` ahora actualizan automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente la base de datos en tiempo real para garantizar persistencia absoluta.
    *   **CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de ID en Citados de Grupo:** Se solucionÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un bug crÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­tico donde responder a un mensaje de grupo con `!add admin` o `!registrar` extraÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a errÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³neamente el JID del chat del grupo (`xxxx@g.us`) a travÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©s de `quoted.from`, registrando o agregando el ID de grupo completo (`5959823815251611282780`) en lugar del nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero del jugador. Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ cambiando el objetivo para priorizar `quoted.author` (el emisor real del mensaje dentro del grupo) con fallback a `quoted.from` (en chats directos).
    *   **Mejora de UX en !registrar AutÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³nomo:** Se optimizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el flujo de error cuando un administrador ejecuta el comando `!registrar` de forma standalone (sin citar a un usuario y con argumentos incompletos). Ahora el bot detecta que no se especificaron los parÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡metros mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­nimos y responde con un mensaje guiado e instructivo que explica detalladamente el formato correcto para ambas opciones (OpciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n A: Respondiendo, OpciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n B: Directo/Manual).
    *   **Censo General de Fichas y Vinculaciones (!censo / !fichas):** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n integrada `getRealmCensus` en `kingdoom-bot/src/supabase.js` que realiza una consulta unificada de todos los jugadores y sus respectivas fichas de personajes (`character_sheets`). Se expuso el comando exclusivo para administradores `!censo` / `!fichas` en `kingdoom-bot/src/handlers/admin.js`, el cual genera un hermoso y estructurado reporte que detalla: total de aventureros, porcentaje de vinculaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n a WhatsApp, nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero de PJs por usuario, los nombres de cada uno de sus PJs (PJ 1, PJ 2) y, para aquellos pendientes sin ficha completada, calcula automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente el tiempo transcurrido en dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­as desde su registro original con una alerta de advertencia.
    *   **Consistencia y Citados en !grant y !ban:** Se habilitÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el soporte para citar/responder mensajes de WhatsApp en los comandos de administraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `!grant` y `!ban`. Esto permite otorgar oro (ej. `!grant 500` respondiendo al jugador) o banear (ej. `!ban` respondiendo al jugador) directamente sin requerir escribir sus nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmeros de telÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©fono a mano.
*   **Notas/Advertencias:** Todas las modificaciones son 100% compatibles con la base de datos Supabase existente y la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³gica del bot. El compilador TypeScript pasÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ con ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©xito (`Exit code: 0`).
...
*   **Resumen de Tareas:** MigraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n completa de Kingdoom Bot a Railway y soporte de visualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de QR en alta definiciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n para WhatsApp Web.
*   **Cambios Clave:**
    *   **MigraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n a Railway:** Se adaptÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del bot para desplegarse de manera robusta en Railway.app, superando las limitaciones de RAM (512MB) y disco volÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡til del plan gratuito de Render.
    *   **Docker & Volumen Persistente:** Se removiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la directiva `VOLUME` en el `Dockerfile` (no soportada nativamente por Railway) y se configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la persistencia de la sesiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n mediante un disco montado en `/app/.wwebjs_auth` desde la interfaz de Railway.
    *   **Servidor Web QR en HD:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡gina interactiva en `PORT = 8080` (en `src/index.js`) que sirve el cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³digo QR generado como imagen PNG en alta definiciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n, facilitando su escaneo e indicando el estado `ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¢ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ§Ã‚Â¦Ã‚Â¿ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¯Ã‚Â½Ã‚Â¦ Bot Conectado` una vez autenticado.
    *   **RemociÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del comando !daily:** Se removiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ por completo la funcionalidad de reclamo de recompensas diarias (`!daily`), limpiando sus imports, su lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³gica interna de base de datos, la funciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de selecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de premios, su menciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n en el comando `!ayuda` y su registro en la lista de comandos procesados de `index.js`.
    *   **Privilegios de Owner y Administradores:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ un sistema robusto de permisos gestionado en `src/adminStore.js` con persistencia en el volumen de Railway (`/app/.wwebjs_auth/admins.json`). El nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero `595987273405` se definiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ como **Soberano (Owner)** del bot, teniendo acceso exclusivo a comandos para conceder (`!add admin <numero>`) o revocar (`!remove admin <numero>`) roles de administrador.
    *   **RestricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n y Mejoras de !registrar:** El comando `!registrar` ahora estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ restringido ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºnicamente a los administradores y al owner. Otorga **2500 de oro inicial** por defecto, y permite especificar un monto a la derecha (ej. `!registrar pepe 200000`, soportando separadores de miles). AdemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s, se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ soporte UX premium: si se ejecuta respondiendo a un mensaje de WhatsApp, extrae automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente el nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero del remitente del mensaje citado.
    *   **Mensaje de Bienvenida Premium en Dos Partes:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `src/handlers/welcome.js` para enviar dos mensajes secuenciales e interactivos con un intervalo de 1.5s al detectar nuevos miembros en el grupo de WhatsApp. El primer mensaje incluye una caja medieval de bienvenida para `ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ¯Ã‚Â¿Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â­ÃƒÂ¯Ã‚Â¿Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â¿Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¯Ã‚Â¿Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¨ÃƒÂ¯Ã‚Â¿Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ¯Ã‚Â½Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ¯Ã‚Â½Ã‚Â½ ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â°ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ©Ã¯Â¿Â½Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ¨Ã¢â‚¬ÂºÃ¢â‚¬Â¹ y un link directo a su canal de informaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n para crear el primer personaje, mientras que el segundo lista oficialmente a los "Guardianes del Reino" (`Nothing`, `Zoelfrost`, `Ord`, `E.xe`). Incorpora menciones automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticas en alta prioridad a los miembros reciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n unidos.
    *   **ResoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Discrepancias de JID en Paraguay:** Se identificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ que WhatsApp a nivel de servidor aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ade o remueve un dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­gito `9` despuÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©s del cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³digo de paÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­s paraguayo (`595`), resultando en discrepancias de formato JID (ej. `5959987273405@c.us` vs `595987273405@c.us`). Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `src/adminStore.js` para admitir y homologar automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente ambos formatos, permitiendo que seas reconocido como Soberano (Owner) de inmediato.
    *   **IdentificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Remitentes en Grupos de WhatsApp:** Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el error de mapeo donde el bot extraÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a el emisor usando `msg.from` (que en grupos devuelve el ID del grupo en lugar del nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero del remitente). Ahora el bot extrae al emisor real de forma infalible con `msg.author || msg.from`, permitiendo a los administradores ejecutar comandos desde grupos.
    *   **Filtrado Silencioso de Mensajes No Registrados:** Para evitar spam masivo de `Viajero desconocido...` ante palabras cotidianas en grupos y PV, se configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el bot para ignorar de manera silenciosa cualquier mensaje de usuario no registrado que carezca del prefijo de comando `!`.
    *   **CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de ReferenceError en el Handler de Jugadores:** Al refactorizar la identificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de emisores se removiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ accidentalmente la declaraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n local de `chatId` en `src/handlers/player.js` que el historial de chat con Inteligencia Artificial requerÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a. Se reincorporÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `const chatId = msg.from;` restableciendo la persistencia correcta y solventando el crash que arrojaba `ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¢ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â§ÃƒÂ¯Ã‚Â½Ã‚Â¡ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¢â‚¬Å¡Ã¢â‚¬Å¾Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¦Ã¢â‚¬Â¢Ã¢â‚¬Â¢ÃƒÅ½Ã‚Â´ÃƒÂ¯Ã‚Â½Ã‚Â¯ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¸ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â¿Ã‚Â½ El reino esta en llamas...`.
    *   **MenÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âº DinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mico e Inteligente para !ayuda:** Se reprogramÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el comando `!ayuda` en `src/handlers/player.js` para detectar en tiempo real si el remitente del mensaje es el Soberano (Owner) o un Administrador del Reino, anexando de manera dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mica sus comandos exclusivos (como `!registrar`, `!grant`, `!stats`, `!broadcast`, `!admin`, etc.) al menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âº tradicional de juego de WhatsApp.
    *   **Fortalecimiento en NormalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de TelÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©fonos:** Se securizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `normalizePhone` en `src/supabase.js` convirtiendo el argumento de entrada a String y aplicando valores por defecto seguros para prevenir TypeErrors inesperados si el JID o nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero remitente no estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ definido.
    *   **RestricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n Estricta de Respuestas a Prefijo (!):** Para evitar que el bot responda con el Heraldo AI a conversaciones cotidianas de cualquier usuario (incluidos dueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±os y administradores), se configurÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ una regla estricta al inicio del manejador de mensajes de WhatsApp. Si el mensaje no inicia con el prefijo `!`, se ignora de manera inmediata y silenciosa (`if (!hasPrefix) return;`).
    *   **IdentificaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n DinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mica de DueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o por Env y JID de AcompaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ante:** Se adaptÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `isOwner` en `src/adminStore.js` para validar dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡micamente si el nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmero del emisor coincide con la variable de entorno `OWNER_NUMBER` o `ADMIN_NUMBER` definida en Railway, e incorporÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ soporte directo nativo para el identificador de dispositivo acompaÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±ante `275162062668001` como Soberano (Owner).
    *   **NormalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n Unificada de TelÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©fonos Internacionales:** Se unificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la lÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³gica de `normalizePhone` importÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndose desde `adminStore.js` a `supabase.js`. Ahora formatea de forma consistente nÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºmeros de Paraguay (removiendo el 9 adicional si tiene 13 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­gitos), MÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©xico (canonicalizando a `521` si tiene 12 dÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­gitos) y Argentina (canonicalizando a `549` y eliminando el `15` si estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ presente). Esto previene inconsistencias entre los datos guardados en la BD y las llamadas de eventos en WhatsApp.
    *   **Bypass de !ayuda para Nuevos Admins/Usuarios No Registrados:** Se modificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `handlePlayerMessage` en `src/handlers/player.js` para procesar el comando `!ayuda` antes de comprobar si el jugador existe en la BD. Esto permite a los administradores reciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adidos u dueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±os ver el menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âº, identificar sus roles y diagnosticar su telÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©fono con una nota explicativa sobre cÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³mo registrarse, en lugar de recibir el mensaje de "Viajero desconocido".
    *   **Registro de Handoff:** Se registrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ formalmente el estado y las instrucciones del bot en la memoria compartida (`kingdoom-memory` MCP) para sincronizar el trabajo con Codex.
*   **Notas/Advertencias:** El bot estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ activo y online. Solo requiere escanear el QR generado en su dominio pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºblico de Railway. El cambio de la remociÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del !daily, la reestructuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de permisos/registro, la bienvenida en dos partes, la correcciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de JIDs/mensajerÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­a grupal, el menÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âº dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mico de ayuda, la restricciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n estricta de prefijos, el diagnÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³stico de identidad, el JID especÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­fico del dueÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o, la normalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n unificada de telÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©fonos internacionales y el bypass de ayuda fue committeado y pusheado de inmediato para gatillar el despliegue automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tico en Railway.
...
*   **Resumen de Tareas:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la capacidad de dar oro a mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºltiples jugadores simultÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡neamente ("add_multiple_players_gold").
*   **Cambios Clave:**
    *   **AcciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de lista:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `add_multiple_players_gold` para procesar una lista de nombres de usuario.
    *   **BÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsqueda flexible:** El motor busca a los jugadores indicados ignorando mayÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsculas/minÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsculas y buscando coincidencias parciales, igual que en la bÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsqueda individual.
    *   **Prompts:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el prompt de IA para utilizar un payload de la forma `{"usernames": ["User A", "User B"], "amount": X}` cuando se le piden varios nombres.
*   **Notas/Advertencias:** Validado con `npx tsc --noEmit` y `npm run build`.

### [2026-05-18] - [Autor: Antigravity]
*   **Archivos Modificados:** `src/features/archivist/archivistActions.ts`, `src/features/archivist/archivist.types.ts`, `api/admin/ask-archivist.ts`, `api/admin/_aiPrompts.ts`, `AI_CHANGELOG.md`
*   **Resumen de Tareas:** Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la capacidad de dar oro a todos los jugadores ("add_all_players_gold") desde el Archivista.
*   **Cambios Clave:**
    *   **AcciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n global:** Se implementÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `add_all_players_gold` en el motor de acciones del Archivista, permitiendo actualizar a todos los jugadores del contexto en una sola solicitud.
    *   **Prompts:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el prompt de IA para reconocer comandos globales y emitir un payload simple de `{ "amount": X }` sin requerir nombre de usuario.
*   **Notas/Advertencias:** Validado con `npx tsc --noEmit` y `npm run build`.
...
*   **Archivos Modificados:** `src/features/archivist/archivistLive.ts`, `src/components/ArchivistSection.tsx`, `AI_CHANGELOG.md`
*   **Resumen de Tareas:** FinalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del fix para el ranking de oro del Archivista y refuerzo semÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ntico.
*   **Cambios Clave:**
    *   **Contexto Admin:** Se verificÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la inclusiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de `richestPlayers` en el resumen runtime para staff.
    *   **Refuerzo SemÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ntico:** Se eliminÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `oro` de `CARD_STOPWORDS` y se duplicÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el `categoryBoost` para asegurar que las tarjetas de jugadores tengan prioridad absoluta en consultas econÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³micas.
    *   **DetecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de IntenciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Se flexibilizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `isPlayerGoldQuestion` para detectar "ranking", "ricos" y "riqueza" sin necesidad de mencionar explÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â­citamente "jugador".
*   **Notas/Advertencias:** Validado con `npm run build`. El sistema ahora diferencia correctamente entre "comprar oro" (mercado) y "ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÂ¯Ã‚Â¿Ã‚Â½Ãƒâ€šÃ¢â€šÂ¬ÃƒÂ¥Ã‚Â Ã¢â€žÂ¢ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¿quiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n tiene mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s oro?" (jugadores).
...
*   **Resumen de Tareas:** ReconstrucciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del Archivista hacia un formato de chat puro con contexto vivo del reino, tarjetas compactas y preparaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n/ejecuciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de acciones admin por confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n conversacional.
*   **Cambios Clave:**
    *   **Chat puro:** Se eliminÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la estructura anterior con panel lateral y controles de memoria visibles para concentrar toda la experiencia en una sola interfaz conversacional.
    *   **Contexto vivo:** El Archivista ahora resume mercado, misiones, eventos, grimorio, biblioteca y, en modo admin, tambiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©n jugadores cargados.
    *   **Tarjetas compactas:** Las respuestas pueden adjuntar tarjetas breves de mercado, eventos, misiones, magias, bestiario, flora y documentos sin romper la versiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³vil.
    *   **Modo admin real:** Se integrÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el borrador y la ejecuciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de acciones del reino tras confirmaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n `si/no` en el chat para oro, misiones, eventos, mercado, magia, bestiario, flora y documentos.
    *   **Cache y contexto:** Se ajustÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el endpoint para separar respuestas pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºblicas/admin y considerar el resumen vivo del reino al generar la respuesta IA.
*   **Notas/Advertencias:** El Archivista sigue dependiendo de las APIs/configuraciones IA ya existentes. Conviene validar flujo pÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºblico y flujo admin tras cada redeploy porque ahora la capa operativa ya no es solo informativa.
...
*   **Resumen de Tareas:** ResoluciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de problemas de visualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de enlaces, rediseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o de la interfaz de reproducciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n/descarga para mayor compacidad y limpieza de la secciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del Archivista.
*   **Cambios Clave:**
    *   **Limpieza de Interfaz (ArchivistSection):** EliminaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de la descripciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n redundante en la cabecera del Archivista ("Consulta el reino..."), siguiendo el rediseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o hacia un formato de chat puro.
    *   **UI Minimalista (AnimeHubSection):** SustituciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n del selector de servidores por un componente ultra-compacto con icono de flecha (`ChevronDown`), optimizando el espacio en la consola de acciones.
    *   **NormalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Enlaces:** Se actualizÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `normalizeLinks` para soportar arrays directos de `servers` y `downloads` que devuelven los scrapers actuales.
    *   **CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Mapeo (AnimeFLV):** Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ `fetchAnimeFlvLinks` para procesar correctamente el payload envuelto de la API.
    *   **Seguridad:** MigraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n total de API Keys hardcodeadas a la constante `ANIME_HUB_API_KEY`.
    *   **Proxy API:** ActualizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de `api/anime/proxy.ts` para mejorar la compatibilidad del mapeo de fuentes y enlaces.
*   **Notas/Advertencias:** La interfaz ahora es mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s limpia y evita solapamientos en resoluciones bajas o mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³viles.
...
*   **Resumen de Tareas:** FinalizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de la integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de AnimeFLV, implementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de filtros por proveedor y optimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de conectividad (CORS/Timeouts).
*   **Cambios Clave:**
    *   **Endpoints:** OptimizaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de proxies en Vercel para streaming y descargas; ahora aceptan `ANIMEFLV_API_URL` como variable server-side.
    *   **UI Web/MÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³vil:** ImplementaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de selectores de proveedor y filtros dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡micos en ambas plataformas.
    *   **Conectividad:** CorrecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de tipos TypeScript para `ApiRequest` (aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adido `query`) e inclusiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de declaraciones globales para entornos Node.js.
    *   **Robustez:** InyecciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de `User-Agent` real en peticiones de backend para evitar bloqueos 403 y timeouts de 8s con `AbortController`.
    *   **ConfiguraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n:** Documentada la nueva variable `ANIMEFLV_API_URL` en `.env.example`.
*   **Notas/Advertencias:** La integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n es ahora resiliente a fallos de red y cumple con los estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ndares de tipado de Vercel. Se requiere redeploy final.
...
*   **Resumen de Tareas:** Se ampliÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ Anime Hub para aprovechar mejor las capacidades reales de `anime1v` con filtros por proveedor, enlaces mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s completos y una experiencia coherente entre web y app.
*   **Cambios Clave:**
    *   Se aÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±adieron filtros reales por proveedor `anime1v` (`AnimeAV1`, `AnimeFLV`, `TioAnime`, `JKAnime`, `HentaiLA`, `MonosChinos`) en web y mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³vil.
    *   La bÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsqueda ahora puede forzar el dominio correcto en `anime1v`, en lugar de tratarlo como una ÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºnica fuente genÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â©rica.
    ...
    *   Se corrigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ la referencia del endpoint batch a `/api/v1/anime/batch-download` para mantenerla alineada con el backend real.
*   **Notas/Advertencias:** Validar con `npx tsc --noEmit` y `npm run build` antes de publicar. Los proveedores mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡s allÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ de `AnimeAV1` siguen dependiendo de que el backend `anime1v` tenga esos scrapers y requisitos externos operativos.
...
*   **Resumen de Tareas:** Finalizada la integraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de Anime Hub con soporte completo para reproducciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n (streaming) y descargas directas desde la API real.
*   **Cambios Clave:**
    *   **Tipado:** Extendida la interfaz `AnimeHubProvider` con `getEpisodeLinks` para soportar la obtenciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡mica de servidores.
    ...
    *   **UI Web:** `AnimeHubSection` ahora permite seleccionar episodios, cargando dinÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡micamente los servidores de streaming y enlaces de descarga en un panel integrado.
    ...
    *   **API Hotfix:** Se redirigiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el almacenamiento temporal de la API para evitar errores 500 en entornos serverless (Vercel).
*   **Notas/Advertencias:** Validado y sincronizado en GitHub. El sistema mantiene fallback automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tico al modo cascarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n si la API no responde.
...
*   **Resumen de Tareas:** ConexiÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n real del mÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³dulo Anime Hub con `anime1v-api` mediante variables de entorno, manteniendo el modo cascarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n como fallback seguro.
*   **Cambios Clave:**
    ...
    *   **Web:** `AnimeHubSection` ahora detecta automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ticamente si existe la URL de la API para conmutar entre el proveedor mock y el remoto, con manejo de errores elegante en el feedback.
    *   **Mobile:** Actualizado `animeHubProvider.ts` para consumir `EXPO_PUBLIC_ANIME_HUB_API_URL` si estÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡ presente, integrando los flujos de bÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Âºsqueda e informaciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n real con fallback automÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â¡tico al shell mock en caso de error o ausencia de configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n.
    *   **Resiliencia:** Se preservÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³ el diseÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â±o premium y el funcionamiento del modo cascarÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n para entornos sin configuraciÃƒÂ¯Ã‚Â¾Ã†â€™ÃƒÅ½Ã‚Â¸ÃƒÂ¦Ã‚Â´Ã‚Â¥ÃƒÂ£Ã¯Â¿Â½Ã‚Â¤ÃƒÂ¯Ã‚Â½Ã‚Â³n de API.
...
*   **Resumen de Tareas:** CorrecciÃ³n de sintaxis de parser de PowerShell en el script de instalaciÃ³n de Graphify.
*   **Cambios Clave:**
    *   Se moviÃ³ el bloque `param(...)` a la primera lÃ­nea del archivo. En PowerShell, el bloque de parÃ¡metros debe ser la primera sentencia ejecutable en el script; colocar asignaciones de variables antes de `param` causaba un error de sintaxis del parser (`ParserError: InvalidLeftHandSide`).
...
*   **Archivos Modificados:** `AGENTS.md`, `AI_CHANGELOG.md`, `ai-memory/kingdoom-memory.jsonl`
*   **Resumen de Tareas:** ExpansiÃ³n de reglas y utilidades de Graphify en el manual para agentes.
*   **Cambios Clave:**
