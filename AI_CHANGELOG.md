...
*   **Cambios Clave:**
    ...
    *   **[CRÍTICO] Fix `!broadcast` — 100% fallido sin diagnóstico (`admin.js`):** El comando fallaba silenciosamente para todos los jugadores (0 exitosos, N fallidos) porque: (1) no validaba si el número estaba registrado en WhatsApp antes de enviar (`client.isRegisteredUser()`), y (2) el `catch` vacío tragaba el error sin loggear nada, imposibilitando el diagnóstico. Se corrigió agregando validación previa, logging detallado del error, y un reporte que lista los nombres de los jugadores que fallaron con la razón (ej: `no WA`).
