...
function getLocalSheets(): CharacterSheet[] {
  ...
  if (stored) {
    try {
      ...
      console.error("Failed to parse character sheets", e instanceof Error ? e.message : String((e as any)?.message ?? e));
      return [];
    }
  }
  return [];
...
export async function getCharacterSheets(): Promise<CharacterSheet[]> {
  ...
  if (error) {
    console.error("Supabase error fetching sheets:", error instanceof Error ? error.message : String((error as any)?.message ?? error));
...
export async function saveCharacterSheet(sheet: CharacterSheet): Promise<void> {
  ...
  if (error) {
    console.error("Supabase error saving sheet:", error instanceof Error ? error.message : String((error as any)?.message ?? error));
...
export async function deleteCharacterSheet(id: string, portraitUrl?: string): Promise<void> {
  ...
  if (error) {
    console.error("Supabase error deleting sheet:", error instanceof Error ? error.message : String((error as any)?.message ?? error));
...
export async function getPlayerSheets(playerId: string): Promise<CharacterSheet[]> {
  ...
  if (error) {
    console.error("Supabase error fetching player sheets:", error instanceof Error ? error.message : String((error as any)?.message ?? error));
