...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  const { id, provider } = req.query as any;
  ...
  try {
    if (provider === "animeflv") {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ message: "Error al consultar AnimeFLV (Detail)." });
      }
    ...
    return res.status(400).json({ message: "Proveedor no soportado para detalle." });
  } catch (error) {
    return res.status(500).json({ 
      message: "Error interno al procesar detalle de anime.",
