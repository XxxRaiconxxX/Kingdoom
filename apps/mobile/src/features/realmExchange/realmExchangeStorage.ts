...
function getLegacyStorageKeys(playerId: string) {
  return [
    `kingdoom.native.realm-exchange.v1.${playerId}`,
    `kingdoom.native.realm-exchange.v${STATE_VERSION}.${playerId}`,
  ];
}
...
async function loadLegacyLocalState(playerId: string): Promise<RealmExchangePlayerState> {
...
async function saveLegacyLocalState(
  playerId: string,
...
async function clearLegacyLocalState(playerId: string) {
  await Promise.all(
    getLegacyStorageKeys(playerId).map((key) => AsyncStorage.removeItem(key))
  );
}
...
export async function loadExchangeState(playerId: string): Promise<RealmExchangePlayerState> {
  const localState = await loadLegacyLocalState(playerId);
...
export async function saveExchangeState(playerId: string, state: RealmExchangePlayerState) {
  ...
  if (!synced) {
    await saveLegacyLocalState(playerId, state);
    return false;
  }
...
export async function openAssetPredictionSecure(input: {
  ...
  if (!supabase) {
    return {
      status: "error" as const,
      message: "Supabase no esta configurado para abrir predicciones.",
      state: input.state,
      nextGold: input.gold,
    };
  }
...
export async function resolveRealmExchangePredictionsSecure(input: {
  ...
  if (!supabase) {
    return {
      status: "error" as const,
      message: "Supabase no esta configurado para resolver predicciones.",
      state: input.state,
      nextGold: null,
      payoutGold: 0,
      remoteApplied: false,
