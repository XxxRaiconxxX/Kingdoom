...
- **Impact Analysis (affected):** Before modifying shared functions, utils, types, or database schemas, run `graphify affected "<symbol_or_file>"` to identify all components, handlers, or pages that depend on it and prevent regression bugs.
- **Inter-Repo Tracing (global):** Use the global graph merge (`graphify global list` / `graphify global path`) to trace dependencies and data contracts that cross the boundary between the web client (`Kingdoom-sync`) and the WhatsApp bot (`kingdoom-bot`).
- **Flow Visualizations:** For documenting complex transaction flows (like auctions escrow or tavern minigames), use `graphify export callflow-html` or `graphify tree` to generate interactive architecture/flow maps.
