...
import { Box, Coins, RefreshCcw, HandCoins, AlertOctagon } from "lucide-react";
...
export function TavernGame() {
  ...
  const openChest = (index: number) => {
    ...
    let actualResult: ResultType = "x0"; // Default / Mimic
    ...
    // Populate the other chests magically so there is exactly one x2, one x1, one x0 visually
    // (This is just visual flavor, the math already triggered)
    ...
    if (actualResult === "x2") {
      ...
      // x0 (Mimic)
      setConsecutiveSpins(0); // Restarts difficulty
    }
  };
    ...
    <div className="rounded-[2rem] border border-stone-800 bg-stone-900/80 p-6 md:p-8 relative overflow-hidden shadow-[inset_0_4px_30px_rgba(0,0,0,0.5)]">
            ...
            <div className="flex flex-col items-center py-4">
              ...
              <div className="grid grid-cols-3 gap-3 md:gap-6 w-full max-w-lg">
                {[0, 1, 2].map((i) => {
                  ...
                  let chestClass = "border-stone-700 bg-stone-900 hover:border-amber-500/50 hover:bg-stone-800";
                  ...
                  if (phase === "revealed") {
                    if (res === "x2") {
                      ...
                      chestClass = "border-rose-600 bg-rose-950/40 shadow-[0_0_20px_rgba(225,29,72,0.2)] opacity-70";
                  ...
                  return (
                    <motion.button
                      ...
                      className={`relative flex aspect-square flex-col items-center justify-center rounded-2xl border-2 transition-all duration-500 ${chestClass}`}
                    >
                      {phase === "playing" ? (
                        <>
                          <Box className="h-10 w-10 text-stone-400 md:h-12 md:w-12" />
                          <div className="absolute inset-0 rounded-2xl bg-gradient-to-t from-stone-900/80 to-transparent" />
                        </>
                      ) : (
                        <motion.div
                        ...
                        >
                          {res === "x2" && (
                            <>
                              <Coins className="mb-2 h-10 w-10 text-emerald-400 dr" />
                              <span className="text-xl font-black text-emerald-400 drop-shadow-md">WIN x2</span>
                            </>
                          )}
                          {res === "x1" && (
                            <>
                              <RefreshCcw className="mb-2 h-8 w-8 text-amber-400" />
                              <span className="text-lg font-bold text-amber-400">Recuperas</span>
                            </>
                          )}
                          {res === "x0" && (
                            <>
                              <AlertOctagon className="mb-2 h-8 w-8 text-rose-500" />
                              <span className="text-xl font-black text-rose-500">MIMIC</span>
                            </>
                          )}
                        </motion.div>
                      )}
              ...
              {phase === "revealed" && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-8 space-y-4 w-full">
                  <button
                    onClick={resetGame}
                  ...
                  <div className="text-center">
                    <p className="text-xs text-stone-500 font-semibold uppercase tracking-widest">Apuesta actual: {bet} | Dificultad: Fase {difficultyLevel + 1}</p>
                  </div>
                </motion.div>
              )}
            </div>
      ...
      <AnimatePresence>
        {showCashout && balance && balance > 0 && (
          <TavernCashoutModal balance={balance} onClose={() => setShowCashout(false)} onSuccess={handleCashoutComplete} />
        )}
      </AnimatePresence>
    </div>
  );
