...
- For architecture review, debugging, audits, impact analysis, or handoff work, prefer Graphify before broad manual browsing. Only fall back to raw grep/file-by-file reading when Graphify does not surface enough context.
- For UI to data flow questions in this repo, prefer Graphify first for traces such as component -> context/hook -> util/service -> Supabase.
- Before changing logic tied to roleplay access, market, minigames, archivist flows, or admin panels, use Graphify to identify nearby files and dependencies if graphify-out/graph.json exists.
