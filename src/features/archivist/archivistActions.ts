...
import { createPlayerAccount, updatePlayerGold, addMultiplePlayersGold } from "../../utils/players";
...
async function executePlayerAction(
  ...
  if (draft.kind === "add_all_players_gold") {
    ...
    await addMultiplePlayersGold(context.players.map(p => p.id), amount);
    return {
      status: "success",
  ...
  if (draft.kind === "add_multiple_players_gold") {
    ...
    await addMultiplePlayersGold(foundPlayers.map(p => p.id), amount);
