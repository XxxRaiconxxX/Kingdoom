...
async function fetchJson<T = any>(url: string, headers?: HeadersInit, timeout = 8000) {
  ...
  try {
    ...
    clearTimeout(id);
    if (!response.ok) {
      console.warn(`Fetch error: ${response.status} for ${url.split("?")[0]}`);
      return null;
    }
    ...
    clearTimeout(id);
    console.error(`Fetch exception for ${url.split("?")[0]}:`, error instanceof Error ? error.message : String(error));
    return null;
  }
}
