...
function getLocalSheets(): CharacterSheet[] {
  ...
  if (stored) {
    try {
      ...
      console.error("Failed to parse character sheets", e instanceof Error ? e.message : (e && typeof e === 'object' && 'message' in e ? String((e as any).message) : String(e)));
      return [];
    }
  }
  return [];
...
export async function getCharacterSheets(): Promise<CharacterSheet[]> {
  ...
  if (error) {
    console.error("Supabase error fetching sheets:", error instanceof Error ? error.message : (error && typeof error === 'object' && 'message' in error ? String((error as any).message) : String(error)));
...
export async function saveCharacterSheet(sheet: CharacterSheet): Promise<void> {
  ...
  if (error) {
    console.error("Supabase error saving sheet:", error instanceof Error ? error.message : (error && typeof error === 'object' && 'message' in error ? String((error as any).message) : String(error)));
...
export async function deleteCharacterSheet(id: string, portraitUrl?: string): Promise<void> {
  ...
  if (error) {
    console.error("Supabase error deleting sheet:", error instanceof Error ? error.message : (error && typeof error === 'object' && 'message' in error ? String((error as any).message) : String(error)));
...
export async function getPlayerSheets(playerId: string): Promise<CharacterSheet[]> {
  ...
  if (error) {
    console.error("Supabase error fetching player sheets:", error instanceof Error ? error.message : (error && typeof error === 'object' && 'message' in error ? String((error as any).message) : String(error)));
