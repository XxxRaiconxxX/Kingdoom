...
*   **Cambios Clave:**
    ...
    *   **Identificación Dinámica de Dueño por Env:** Se adaptó `isOwner` en `src/adminStore.js` para validar dinámicamente si el número del emisor coincide con la variable de entorno `OWNER_NUMBER` o `ADMIN_NUMBER` definida en Railway, además de la lista hardcodeada habitual.
...
*   **Notas/Advertencias:** El bot está activo y online. Solo requiere escanear el QR generado en su dominio público de Railway. El cambio de la remoción del !daily, la reestructuración de permisos/registro, la bienvenida en dos partes, la corrección de JIDs/mensajería grupal, el menú dinámico de ayuda, la restricción estricta de prefijos y el diagnóstico de identidad fue committeado y pusheado de inmediato para gatillar el despliegue automático en Railway.
