...
export function KingdoomAuthModal({
  ...
  async function handleFirstAccess(event: React.FormEvent) {
    event.preventDefault();
    if (!username.trim()) { setFeedback("Escribe tu nombre de usuario."); return; }
    ...
    setBusy(true); setFeedback("Verificando el codigo de WhatsApp...");
    try {
      const { error } = await Promise.race([activateKingdoomAccount({ username, password, code }), new Promise<never>((_, reject) => setTimeout(() => reject(new Error("La validacion tardo demasiado. Comprueba la conexion e intentalo de nuevo.")), 15000))]);
      if (error) throw error;
      await finish(username);
    } catch (error) {
  ...
  async function handleStaffReset(event: React.FormEvent) {
    ...
    try {
      ...
      const authenticatedStaff = Boolean(session?.access_token && !isAnonymous);
      const action = authenticatedStaff ? "reset" : "recover";
      if (action === "recover" && !/^\d{6}$/.test(code)) {
        throw new Error("Para recuperar un admin sin iniciar sesión, escribe el código de 6 dígitos de WhatsApp.");
      }
      const response = await fetch("/api/auth/staff", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(session?.access_token && authenticatedStaff ? { Authorization: `Bearer ${session.access_token}` } : {}) },
        body: JSON.stringify({ action, username: resetTarget, password: resetPassword, code: code.trim() }),
      });
  ...
  return createPortal(
    <AnimatePresence>
      <motion.div className="fixed inset-0 z-[999] flex items-start justify-center overflow-y-auto bg-black/75 p-4 backdrop-blur-sm sm:items-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="kingdoom-auth-title" className="realm-auth-dialog relative my-4 w-full max-w-lg rounded-[2rem] border border-amber-400/25 bg-stone-950 px-5 pb-6 pt-8 text-stone-100 shadow-2xl shadow-black/60 sm:my-8 sm:px-8 sm:pb-8" initial={{ opacity: 0, y: 24, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }}>
          <div className="flex min-w-0 items-start justify-between gap-4"><div><p className="absolute -top-4 left-6 whitespace-nowrap rounded-full border border-amber-400/25 bg-stone-950 px-3 py-1 text-xs font-semibold uppercase tracking-[.22em] text-amber-300/80">Kingdoom · acceso seguro</p><h2 id="kingdoom-auth-title" className="mt-2 text-2xl font-black">{title}</h2></div><button type="button" onClick={onClose} aria-label="Cerrar" className="min-h-11 min-w-11 rounded-full p-2 text-stone-400 hover:bg-stone-800 hover:text-stone-100"><X size={20} /></button></div>
          {mode === "login" && <form className="mt-7 space-y-4" onSubmit={handleLogin}><label className="block text-sm text-stone-300">Nombre de usuario<input className={`${field} mt-2`} value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required /></label><label className="block text-sm text-stone-300">Contraseña<div className="relative mt-2"><input className={`${field} pr-12`} value={password} onChange={(event) => setPassword(event.target.value)} type={showPassword ? "text" : "password"} autoComplete="current-password" required /><button type="button" aria-label={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"} onClick={() => setShowPassword((value) => !value)} className="absolute right-0 top-0 grid h-full min-w-11 place-items-center text-stone-400">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></label><label className="flex items-center gap-2 text-xs text-stone-300"><input type="checkbox" checked={remember} onChange={(event) => setRemember(event.target.checked)} className="accent-amber-400" />Recordar usuario</label><button disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber-400 px-4 py-3 font-bold text-stone-950 disabled:opacity-60"><KeyRound size={17} />{busy ? "Verificando..." : "Entrar"}</button></form>}
          {mode === "first-access" && <form noValidate className="mt-7 space-y-4" onSubmit={handleFirstAccess}><p className="text-sm leading-6 text-stone-400">Pide tu código con <code className="text-amber-300">!codigo</code> en el chat privado del bot.</p><label className="block text-sm text-stone-300">Usuario<input className={`${field} mt-2`} value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" required /></label><div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm text-stone-300">Nueva contraseña<input className={`${field} mt-2`} value={password} onChange={(event) => setPassword(event.target.value)} type="password" required /></label><label className="block text-sm text-stone-300">Repite la contraseña<input className={`${field} mt-2`} value={passwordRepeat} onChange={(event) => setPasswordRepeat(event.target.value)} type="password" required /></label></div><label className="block text-sm text-stone-300">Código WhatsApp<input className={`${field} mt-2 text-center tracking-[.4em]`} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" required /></label><button type="submit" disabled={busy} aria-busy={busy} className="w-full rounded-xl bg-amber-400 px-4 py-3 font-bold text-stone-950 transition-opacity duration-200 disabled:cursor-wait disabled:opacity-50">{busy ? "Creando acceso..." : "Crear acceso"}</button></form>}
          {mode === "staff-reset" && <form className="mt-7 space-y-4" onSubmit={handleStaffReset}><div className="flex items-center gap-3 rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-3 text-sm text-cyan-100"><ShieldCheck size={18} /><span>Un admin autenticado puede restablecer cuentas. Si olvidaste la contraseña de un admin, usa su código WhatsApp.</span></div><label className="block text-sm text-stone-300">Usuario a restablecer<input className={`${field} mt-2`} value={resetTarget} onChange={(event) => setResetTarget(event.target.value)} required /></label><label className="block text-sm text-stone-300">Nueva contraseña temporal<input className={`${field} mt-2`} value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} type="password" minLength={8} required /></label><label className="block text-sm text-stone-300">Código WhatsApp <span className="text-stone-500">(recuperación sin sesión)</span><input className={`${field} mt-2 text-center tracking-[.4em]`} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} inputMode="numeric" autoComplete="one-time-code" /></label><button disabled={busy} className="w-full rounded-xl bg-amber-400 px-4 py-3 font-bold text-stone-950 disabled:opacity-60">{busy ? "Guardando..." : "Restablecer contraseña"}</button></form>}
          ...
          <div className="realm-auth-links mt-6 flex flex-wrap gap-3 text-xs text-stone-400"><button type="button" onClick={() => { setMode("login"); setFeedback(""); }}>Iniciar sesión</button><button type="button" onClick={() => { setMode("first-access"); setFeedback(""); }}>Primer acceso</button><button type="button" onClick={() => { setMode("staff-reset"); setFeedback(""); }}>Reset staff</button></div>
        </motion.section>
      </motion.div>
    </AnimatePresence>,
    document.body,
