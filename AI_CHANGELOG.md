...
## [Unreleased] - Add tests for pveProgress utilities
- Added a vitest test suite `src/utils/pveProgress.test.ts` to test the pure computation functions `getExperienceForNextLevel`, `getTotalExperienceForLevel`, and `getLevelFromExperience`.
- Configured the `npm run test` script to use vitest.
